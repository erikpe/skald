...
Skald is currently an exploratory language design. The repository baseline (M0), source/diagnostic/lexer phase (M1), source AST/parser phase (M2), and declaration/resolution phase (M3) are implemented; type checking and all later compiler phases remain future work. The language specification remains a draft, and syntax, semantics, and implementation interfaces may change as vertical slices are implemented and tested.
