...
fn disabled_pass_names_are_validated_once_in_deterministic_order() {
    ...
    let expected = "unknown MIR pass names: `missing-pass`, `zeta-pass`; known MIR passes: `checked-integer-constant-folding`, `conservative-cfg-cleanup`, `dead-pure-definition-elimination`, `post-proof-unreachable-block-elimination`, `primitive-algebraic-simplification`, `primitive-constant-folding`, `whole-world-reachability`";
