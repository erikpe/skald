...
pub(crate) fn lower_generic_source_to_preliminary_mir(
    ...
    lower_preliminary_hir(
        &checked
            .hir
            .expect("successful generic type checking must produce typed HIR"),
    )
}
...
mod tests {
    ...
    fn canonical_standard_library_closure_is_complete_and_overridable() {
        ...
        assert_eq!(canonical[8].0, "std/lang.ska");
        assert_eq!(canonical[10].0, "std/test.ska");
        assert_eq!(canonical[11].0, "std/vec.ska");
        assert_eq!(
            canonical
                .iter()
                .map(|(path, _)| *path)
