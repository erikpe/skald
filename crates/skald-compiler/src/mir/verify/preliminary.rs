...
pub(crate) fn initializes_static_field(
    ...
    match instruction {
        ...
        MirInstruction::AggregateOptionalInitialize(operation) => exact(&operation.destination),
        MirInstruction::AggregateOptionalPublish(operation) => exact(&operation.destination),
