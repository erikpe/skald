...
pub(crate) fn initializes_static_field(
    ...
    match instruction {
        ...
        MirInstruction::NestedOptionalInitialize(operation) => exact(&operation.destination),
        MirInstruction::NestedOptionalPublish(operation) => exact(&operation.destination),
