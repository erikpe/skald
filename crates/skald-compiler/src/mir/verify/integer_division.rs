...
fn storage_writes(function: MirDefinitionRef<'_>, storage: StorageId) -> Vec<BlockId> {
    function
        .body()
        .blocks
        .iter()
...
fn dominates(function: MirDefinitionRef<'_>, dominator: BlockId, target: BlockId) -> bool {
    if dominator == target {
        return true;
    }
