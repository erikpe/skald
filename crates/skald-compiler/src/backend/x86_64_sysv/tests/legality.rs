...
fn malformed_integer_casts_are_rejected_at_the_verifier_boundary() {
    ...
    let operation = function.body.blocks[0]
        ...
        .find_map(|instruction| match instruction {
            MirInstruction::Assign(MirAssignment {
                rvalue:
                    MirRvalue {
                        kind: MirRvalueKind::PrimitiveCast { operation, .. },
                        ..
                    },
                ..
            }) => Some(operation),
        ...
        .expect("cast source must lower to a primitive-cast rvalue");
    operation.source = crate::mir::MirPrimitiveType::I64;
    ...
    assert!(error
        .message()
        .contains("primitive cast source is not `i64`"));
}
