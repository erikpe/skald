...
const COPY_SOURCE: &str = concat!(
    ...
    "  copy(ref other: Value) { self.small = other.small; self.value = other.value; self.ratio = other.ratio; self.enabled = other.enabled; }\n",
...
fn preserves_live_aliases_and_scalar_homes_across_copy_calls() {
    let output = assembly(concat!(
        ...
        "  copy(ref other: Value) { self.value = other.value + 5; }\n",
        "}\n",
