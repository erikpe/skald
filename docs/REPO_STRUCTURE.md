...
M3 implements resolution as declaration collection followed by body resolution. Its separate resolved representation has a dense source-ordered function declaration table, a separately indexed definition table, owner-qualified parameter and local IDs, ID-based binding uses, and ID-based direct calls. Declarations own names, signatures, and linkage; definitions own locals and bodies. Public tables support lookup by ID but intentionally provide no name-based declaration-selection API. The `main` name is resolved once into an optional entry candidate; M4 consumes that ID to validate the entry signature.
...
System V integer parameters arrive in `%rdi`, `%rsi`, `%rdx`, `%rcx`, `%r8`, and `%r9`, followed by stack arguments. Parameters are spilled into their frame homes on entry, and `i64` results are returned in `%rax`; `unit` has no result payload and neither reads nor writes a fictitious return register. Calls reserve an independently 16-byte-aligned outgoing stack area when more than six arguments are present. The backend selects call symbols centrally from declaration linkage: internal definitions use deterministic GNU-local `.Lska_fn_N` symbols that cannot collide with valid exact external identifiers, while external declarations retain their declared symbol. Assembly-shape tests cover the register/stack ABI boundary, frame and scratch-register policy, call linkage, all initial instructions, legality rejection, and acceptance by the system assembler.
