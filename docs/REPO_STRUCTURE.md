...
The implemented M1 lexer returns a token stream and accumulated diagnostics together, preserving `Invalid` tokens for recovery instead of aborting on the first lexical error. T2 replaces the original integer-only path with one numeric scanner and an explicit source-level literal classification shared by tokens, AST, and resolved IR. The lexer retains spellings indirectly through complete spans and does not convert semantic values. It already recognizes the boundaries of contracted `i64`, `u64`, `u8`, and `f64` spellings, but gates every kind except unsuffixed decimal `i64` until its end-to-end milestone; malformed and disabled forms remain one recoverable invalid token. Its lexical contract is recorded in [`grammar/README.md`](../grammar/README.md).
