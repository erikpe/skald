...
| Dead normalized path activations | `passes::analyze_dead_normalized_path_activations` on normalized final MIR; repository workflow in [Local final-MIR redundancy measurement](MIR_REDUNDANCY_MEASUREMENT.md) | exact removable carrier protocols, owned declarations/instructions/load results, blockers, and conservative removal bounds without rewriting |
...
1. Find the exact source field and access span in preliminary MIR and the
   activation dump's field entry.
2. Follow that entry's canonical trigger and `Via` witness back to the selected
   entry, checking conservative call or lifecycle targets along the way.
3. Confirm the same `StaticFieldId` appears in the planned activation
   certificate and coordinator activation order; shutdown must be its exact
   reverse position.
4. At the final-MIR checkpoint, use `reachability_dump` to confirm the active
   storage and coordinator obligations remain execution-reachable.
5. Inspect backend static-planning observations or assembly for the private
   slot, then use a native golden/runtime trace for startup, use, and shutdown
   behavior.
...
plan identifies every ordinary boundary. MIR and assembly never look up a
range by name or contain a range-specific runtime operation: the fused plan is visible only as
...
direct or virtual MIR call prints `readonly produced origin exact(...)`; the
backing temporary remains internally mutable only so production and
destruction can write it. If a produced call instead prints mutable access,
loses the exact complete root, is used after its `end-full-expression`, or
cleans on a skipped path, MIR verification must reject it before backend
lowering.

Direct field selection from a produced exact-class receiver now crosses
resolution as one `ProducedReceiver` plus canonical base and intermediate
...
owner as `shared-anchor`; the corresponding `shared-copy`,
`shared-field-copy`, call result, allocation adopt, or shared cast must precede
the consuming call, and `shared-release`
...
Shared-owner dumps must show the ordinary copy, call, allocation, or cast into
a temporary owner, consumption by `shared-field-initialize`, prefix advance,
and epoch cleanup. Optional shared-owner initialization must likewise precede
advance, and shared outer publication must not be mistaken for element-owner
transfer.
...
Use the implemented declaration suite and the nearest write owners:

```text
cargo test --locked -p skald-compiler final_fields
cargo test --locked -p skald-compiler private_cell
