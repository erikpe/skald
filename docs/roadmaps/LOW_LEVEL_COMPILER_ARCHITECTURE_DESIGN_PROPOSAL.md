...
Its [implementation roadmap](LOW_LEVEL_IR_MODEL_ROADMAP.md) is planned; LI01 is
next. Common model contracts are frozen; concrete target counterparts remain
...
| LA02 | **[LIR model, construction, and verification](LOW_LEVEL_IR_MODEL_DESIGN_PROPOSAL.md):** identities, values, memory, CFG/edges, effects, call representation, mutation rules, and dumps | Accepted/frozen design; [roadmap planned](LOW_LEVEL_IR_MODEL_ROADMAP.md), LI01 next. Executable delivery pending. Required handoff: supported construction/transformation APIs and exact selected-stage requirements | LA01 complete; common model readiness and synthetic target checks in roadmap; concrete native counterparts remain LA03 |
