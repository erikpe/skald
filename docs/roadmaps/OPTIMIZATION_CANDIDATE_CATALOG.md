...
| FMM-13 | [Dead normalized condition-carrier storage cleanup](PROOF_PROVENANCE_NORMALIZATION_DISCOVERIES.md#reclassified-path-activations-lose-their-scalar-spill-origin) | After post-proof block/value elimination; before backend frame planning | Provenance foundation is **Proposed** by a [frozen design](../archive/NORMALIZATION_STABLE_PATH_ACTIVATION_PROVENANCE_DESIGN_PROPOSAL.md) and [planned roadmap](NORMALIZATION_STABLE_PATH_ACTIVATION_PROVENANCE_ROADMAP.md); deletion remains a separate follow-up / **Medium to large** | Medium MIR/frame-size and compile-time value | The final-only activation kind will supply stable provenance, but safe deletion still needs exact load/store/lifetime and attachment analysis |
