...
| FMV-16 | [Convergent callable-local constant propagation](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | Proof-rich final MIR plus one narrow proof-consuming transition at mandatory normalization; shared by primitive, checked-integer, and constant-left logical consumers | In progress; structural observers, checked-carrier certification, the convergent solver, primitive assignment consumer, and bounded algebraic/CFG views are implemented; checked and logical consumers remain ([roadmap](CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)) / **Large** | Low demonstrated direct value in corpus version 1, but high semantic-completeness and enabling value for arbitrarily deep supported expressions and exact short-circuit selection | Must model conditional logical dependencies without requiring a skipped RHS, converge without depth limits, retain selected failures/effects, validate rewrites atomically, and preserve independent pass selection; storage propagation remains limited to exhaustively certified protocol-owned private carriers |
