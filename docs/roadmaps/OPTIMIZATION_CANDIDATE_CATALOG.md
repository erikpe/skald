...
| FMC-17 | [Post-proof unreachable block/value elimination](PROOF_PROVENANCE_NORMALIZATION_DESIGN_PROPOSAL.md#ppn12--validate-the-boundary-with-one-conservative-cfg-canary) | Immediately after mandatory proof normalization; before whole-world reachability | Implemented as a selectable final-stage canary but not yet enabled by `default` ([roadmap](PROOF_PROVENANCE_NORMALIZATION_ROADMAP.md)) / **Medium** | Medium MIR/code-size value and high validation value for the normalized boundary | Retains body entry, static-publication endpoints, lifecycle authority, checked failure reachable from executable edges, and every other permanent semantic root; storage deletion remains excluded |
