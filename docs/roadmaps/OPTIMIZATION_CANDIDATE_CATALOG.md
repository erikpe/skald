...
| FMV-13 | [Primitive integer/boolean constant folding](../compiler/PHASES_AND_IR.md#local-final-mir-simplification) | After an initial dead-pure cleanup; before algebraic simplification, and repeated afterward | Implemented; the assignment consumer uses convergent local facts ([active broadening](CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)) / **Medium** | Medium runtime and code size; high enabling value for CFG cleanup | Exact wrapping/width behavior, unsupported-operation barriers, stable spans/identities, and no floating families; checked/logical structure remains under separate rewrite ownership |
...
| FMV-16 | [Convergent callable-local constant propagation](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | Proof-rich final MIR plus one narrow proof-consuming transition at mandatory normalization; shared by primitive, checked-integer, and constant-left logical consumers | In progress; structural observers, checked-carrier certification, the convergent solver, all three internal consumers, bounded algebraic/CFG views, and atomic proof-transition composition are implemented; production logical selection and evidence remain ([roadmap](CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)) / **Large** | Low demonstrated direct value in corpus version 1, but high semantic-completeness and enabling value for arbitrarily deep supported expressions and exact short-circuit selection | Must model conditional logical dependencies without requiring a skipped RHS, retain selected failures/effects, and preserve independent pass selection; storage propagation remains limited to exhaustively certified protocol-owned private carriers |
...
| FMC-11 | [Constant-left short-circuit logical CFG simplification](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | At a narrow proof-consuming transition composed atomically with mandatory proof normalization; before final CFG cleanup | In progress; the exact internal plan and atomic transition consumer are implemented, while production registration, public selection, metrics, and native evidence remain ([roadmap](CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)); broader identities remain follow-up / **Large** | Medium runtime and code size, plus high completeness value for boolean constant propagation | Preserve left evaluation; skip an unselected RHS without requiring purity; execute a selected RHS exactly once; retain selected failures/effects; avoid persistent logical provenance; keep mandatory normalization profile-independent |
...
1. Implement the planned convergent local constant propagation roadmap before
   adding more constant-folding families.
