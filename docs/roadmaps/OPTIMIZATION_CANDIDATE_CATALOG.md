...
| FMV-05 | Deterministic constant floating comparison | After an exact IEEE constant model; before branch folding | Frozen proposed; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium** | Low to medium runtime and CFG value | Must specify NaN, unordered predicates, signed zero, and raw-bit handling independently of the Rust host |
| FMV-06 | Deterministic constant floating arithmetic | After an exact target-independent IEEE evaluator; before algebraic simplification | Frozen proposed; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Medium runtime and code size in numeric programs | Rounding, NaN result/payload policy, infinities, subnormals, signed zero, host independence, and compile-time cost |
