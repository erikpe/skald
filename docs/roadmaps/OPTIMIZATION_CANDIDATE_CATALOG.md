...
| FMV-07 | Exact constant integer-to-floating conversion | With the IEEE evaluator; before floating arithmetic folding | Frozen proposed; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium to large** | Low to medium runtime | Must implement specified round-to-nearest behavior without inheriting host or target quirks |
...
| FMC-04 | Fold constant checked `f64`-to-integer conversions | After exact IEEE/range evaluation; before CFG cleanup | Frozen proposed; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Low to medium runtime and size | Range, finiteness, truncation, target-width result, exact failure reason, and cast-range diamond must be rewritten together |
