...
| FMV-01 | Raw-bit primitive cast folding | After basic constant folding; before algebraic simplification | Draft design in the [target-independent binary64 proposal](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md) / **Small** | Low to medium runtime and code size | Only truly bit-preserving `u64`/`f64` reinterpretations are simple; must retain raw NaN payloads and result type exactly |
...
| FMV-05 | Deterministic constant floating comparison | After an exact IEEE constant model; before branch folding | Draft design in the [target-independent binary64 proposal](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md) / **Medium** | Low to medium runtime and CFG value | Must specify NaN, unordered predicates, signed zero, and raw-bit handling independently of the Rust host |
| FMV-06 | Deterministic constant floating arithmetic | After an exact target-independent IEEE evaluator; before algebraic simplification | Draft design in the [target-independent binary64 proposal](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md) / **Large** | Medium runtime and code size in numeric programs | Rounding, NaN result/payload policy, infinities, subnormals, signed zero, host independence, and compile-time cost |
| FMV-07 | Exact constant integer-to-floating conversion | With the IEEE evaluator; before floating arithmetic folding | Draft design in the [target-independent binary64 proposal](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md) / **Medium to large** | Low to medium runtime | Must implement specified round-to-nearest behavior without inheriting host or target quirks |
...
| FMC-04 | Fold constant checked `f64`-to-integer conversions | After exact IEEE/range evaluation; before CFG cleanup | Draft design in the [target-independent binary64 proposal](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md) / **Large** | Low to medium runtime and size | Range, finiteness, truncation, target-width result, exact failure reason, and cast-range diamond must be rewritten together |
