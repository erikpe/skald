...
| FMC-07 | [Delete obsolete path-condition and logical-expression proof records](PROOF_PROVENANCE_NORMALIZATION_DESIGN_PROPOSAL.md) | In a mandatory normalization stage after their final semantic verifier; before post-proof CFG passes | In progress ([roadmap](PROOF_PROVENANCE_NORMALIZATION_ROADMAP.md); atomic transaction implemented, production seal transition pending) / **Large** | High architectural value; medium MIR/compile-time reduction | The frozen design uses a one-way proof-rich-to-final seal transition, exact carrier-load lowering, and a distinct normalized verifier |
