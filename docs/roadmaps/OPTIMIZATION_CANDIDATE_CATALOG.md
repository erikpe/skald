...
| FMV-01 | Raw-bit primitive cast folding | After basic constant folding; before algebraic simplification | Frozen proposed; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Small** | Low to medium runtime and code size | Only truly bit-preserving `u64`/`f64` reinterpretations are simple; must retain raw NaN payloads and result type exactly |
...
| FMV-05 | Deterministic constant floating comparison | After an exact IEEE constant model; before branch folding | Frozen proposed; target-independent four-way comparison authority implemented, compiler integration pending; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium** | Low to medium runtime and CFG value | Must preserve explicit NaN/unordered predicates, signed-zero equality, and separation from raw-bit identity |
| FMV-06 | Deterministic constant floating arithmetic | After an exact target-independent IEEE evaluator; before algebraic simplification | Frozen proposed; target-independent nearest-ties-to-even arithmetic authority implemented, compiler integration pending; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Medium runtime and code size in numeric programs | Compiler folding must retain NaN-producing operations while preserving infinities, subnormals, signed zero, host independence, and compile-time cost |
| FMV-07 | Exact constant integer-to-floating conversion | With the IEEE evaluator; before floating arithmetic folding | Frozen proposed; target-independent nearest-ties-to-even conversion authority implemented, compiler integration pending; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium to large** | Low to medium runtime | Compiler integration must preserve source signedness and exact target bits without inheriting host or target quirks |
