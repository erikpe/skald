...
| FMC-04 | Fold constant checked `f64`-to-integer conversions | After exact IEEE/range evaluation; before CFG cleanup | In progress; exact topology, carrier certification, value-or-out-of-range evaluation, and convergent facts implemented read-only; rewrite pending; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Low to medium runtime and size | Range, finiteness, truncation, target-width result, exact failure reason, and cast-range diamond must be rewritten together |
