...
| FMC-08 | [Empty-block forwarding](POST_PROOF_CFG_CANONICALIZATION_DESIGN_PROPOSAL.md#empty-block-forwarding) | After post-proof unreachable deletion; before block merging | Proposed ([roadmap](POST_PROOF_CFG_CANONICALIZATION_ROADMAP.md)) / **Medium** | Medium code size and compile time | The frozen design requires exact predecessor occurrences, transitive cycle-safe resolution, permanent-attachment barriers, preserved incoming terminator roles/spans, and a narrow final-stage CFG capability |
| FMC-09 | [Basic-block merging](POST_PROOF_CFG_CANONICALIZATION_DESIGN_PROPOSAL.md#basic-block-merging) | After proof normalization and empty-block forwarding; before whole-world reachability | Proposed ([roadmap](POST_PROOF_CFG_CANONICALIZATION_ROADMAP.md)) / **Medium** | Medium code size and target input quality | The frozen design requires one total incoming goto edge, block-local value preservation, exact instruction/terminator order, permanent-attachment barriers, deterministic convergence, and a narrow final-stage CFG capability |
