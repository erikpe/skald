...
| FMV-16 | [Convergent callable-local constant propagation](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | Proof-rich final MIR plus one narrow proof-consuming transition at mandatory normalization; shared by primitive, checked-integer, and constant-left logical consumers | Proposed ([roadmap](CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)) / **Large** | Low demonstrated direct value in corpus version 1, but high semantic-completeness and enabling value for arbitrarily deep supported expressions and exact short-circuit selection | Must separate checked topology from value provenance, certify only protocol-owned private carriers, model conditional logical dependencies without requiring a skipped RHS, converge without depth limits, retain selected failures/effects, validate rewrites atomically, and preserve independent pass selection |
