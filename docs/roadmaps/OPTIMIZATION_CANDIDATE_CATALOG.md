...
and material-consumer threshold.
...
| FMC-17 | [Post-proof unreachable block/value elimination](PROOF_PROVENANCE_NORMALIZATION_DESIGN_PROPOSAL.md#ppn12--validate-the-boundary-with-one-conservative-cfg-canary) | Immediately after mandatory proof normalization; before whole-world reachability | In progress ([roadmap](PROOF_PROVENANCE_NORMALIZATION_ROADMAP.md); verifier, normalization transaction, and two-seal transition implemented) / **Medium** | Medium MIR/code-size value and high validation value for the normalized boundary | Must retain body entry, static-publication endpoints, lifecycle authority, checked failure reachable from executable edges, and every other permanent semantic root; storage deletion remains excluded |
...
| FMC-07 | [Delete obsolete path-condition and logical-expression proof records](PROOF_PROVENANCE_NORMALIZATION_DESIGN_PROPOSAL.md) | In a mandatory normalization stage after their final semantic verifier; before post-proof CFG passes | Implemented as a mandatory product transition; remaining stage-aware pipeline work is tracked by the active [roadmap](PROOF_PROVENANCE_NORMALIZATION_ROADMAP.md) / **Large** | High architectural value; medium MIR/compile-time reduction | The implemented boundary uses a one-way proof-rich-to-final seal transition, exact carrier-load lowering, and a distinct normalized verifier |
