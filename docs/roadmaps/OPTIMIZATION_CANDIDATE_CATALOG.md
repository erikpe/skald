...
| FMV-05 | Deterministic constant floating comparison | After an exact IEEE constant model; before branch folding | Implemented through the target-independent four-way comparison authority; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium** | Low to medium runtime and CFG value | Preserves explicit NaN/unordered predicates, signed-zero equality, and separation from raw-bit identity |
| FMV-06 | Deterministic constant floating arithmetic | After an exact target-independent IEEE evaluator; before algebraic simplification | Implemented for non-NaN add/subtract/multiply/divide; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Medium runtime and code size in numeric programs | Retains NaN-producing operations while preserving infinities, subnormals, signed zero, host independence, and compile-time cost |
| FMV-07 | Exact constant integer-to-floating conversion | With the IEEE evaluator; before floating arithmetic folding | Implemented for `i64`, `u64`, `u8`, and `bool`; [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium to large** | Low to medium runtime | Preserves source signedness and exact target bits without inheriting host or target quirks |
...
| FMC-04 | [Fold constant checked `f64`-to-integer conversions](../compiler/PHASES_AND_IR.md#frozen-target-independent-binary64-evaluation-direction) | After exact primitive and checked-integer folding; before repeated primitive/dead/CFG cleanup | Implemented ([driver](../compiler/DRIVER_AND_ARTIFACTS.md#final-mir-optimization-selection), [reporting](../compiler/REPORTING.md#checked-floating-to-integer-protocol-simplification-observation), [testing](../development/TESTING.md#checked-floating-to-integer-protocol-simplification-coverage), [design](TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md)) / **Large** | Low to medium runtime and size | The pass atomically rewrites only fully proven successful `i64`/`u64`/`u8` diamonds, preserves source evaluation and result storage/lifetimes, and retains NaN, infinity, finite range failures, malformed, protected, and insufficiently proven protocols |
