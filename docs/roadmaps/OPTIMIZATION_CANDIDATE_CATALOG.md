...
compared the former narrow FMV-15 carrier-provenance candidate, FMV-02, and
FMV-03 without committing to implement any of them. Its
[frozen measurement contract](../archive/LOCAL_MIR_REDUNDANCY_MEASUREMENT_CONTRACT.md)
...
selects no candidate-specific optimization: none crossed the frozen breadth
and material-consumer threshold. The later frozen FMV-16 design deliberately promotes
a broader completeness and architecture goal; it does not reinterpret that
...
It owns conservative integer/boolean/binary64 constant folding, primitive
...
| FMV-13 | [Primitive integer/boolean/binary64 constant folding](../compiler/PHASES_AND_IR.md#local-final-mir-simplification) | After an initial dead-pure cleanup; before algebraic simplification, and repeated afterward | Implemented; the assignment consumer uses convergent local facts ([delivery](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md), [binary64 extension](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md)) / **Medium** | Medium runtime and code size; high enabling value for CFG cleanup | Exact wrapping/width and binary64-bit behavior, unsupported-operation barriers, stable spans/identities, and retained NaN-producing arithmetic; checked/logical structure remains under separate rewrite ownership |
...
| FMV-16 | [Convergent callable-local constant propagation](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | Proof-rich final MIR plus one narrow proof-consuming transition at mandatory normalization; shared by primitive, checked-integer, and constant-left logical consumers | Implemented; structural observers, checked-carrier certification, the convergent solver, all three production consumers, bounded algebraic/CFG views, atomic proof-transition composition, public selection, metrics, native evidence, and closure hardening are complete ([delivery](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)) / **Large** | Low demonstrated direct value in corpus version 1, but high semantic-completeness and enabling value for arbitrarily deep supported expressions and exact short-circuit selection | Conditional logical dependencies do not require a skipped RHS, selected failures/effects and independent pass selection are retained, and storage propagation remains limited to exhaustively certified protocol-owned private carriers |
| FMV-01 | Raw-bit primitive cast folding | After basic constant folding; before algebraic simplification | Implemented for exact `u64`/`f64` reinterpretation; [design](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Small** | Low to medium runtime and code size | Retains raw NaN payloads and result type exactly |
| FMV-02 | Redundant primitive cast and cast-chain elimination | After constant folding; before dead-pure cleanup | Integer-only algebra, arbitrary-length same-block analysis, guarded rewrite, and one default-profile occurrence implemented ([roadmap](../archive/INTEGER_CAST_CHAIN_CANONICALIZATION_ROADMAP.md)); broader primitive families remain follow-up; originally measured but not selected ([study result](../archive/LOCAL_MIR_REDUNDANCY_MEASUREMENT_REPORT.md#candidate-comparison)) / **Medium** | Low demonstrated value in corpus version 1; bounded optimizer-completeness value for dynamic integer chains; possible medium runtime and MIR-size value if broader evidence appears | Integer chains use exact width-aware composition; nonadjacent instructions and shared intermediates are supported, while boolean, floating, checked, raw-bit, cross-block, malformed, and cyclic provenance remain barriers |
| FMV-03 | Local common-subexpression elimination for non-failing primitive rvalues | After constant/algebraic simplification; before dead-pure cleanup | Follow-up; measured but not selected ([study result](../archive/LOCAL_MIR_REDUNDANCY_MEASUREMENT_REPORT.md#candidate-comparison)) / **Medium** | No demonstrated value for the exact local form in corpus version 1; broader CSE remains unmeasured | Restrict to exact same-block pure operations; source values must dominate; floating equivalence, spans, runtime traces, and checked operations need exclusions |
...
| FMV-05 | Deterministic constant floating comparison | After an exact IEEE constant model; before branch folding | Implemented through the target-independent four-way comparison authority; [design](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium** | Low to medium runtime and CFG value | Preserves explicit NaN/unordered predicates, signed-zero equality, and separation from raw-bit identity |
| FMV-06 | Deterministic constant floating arithmetic | After an exact target-independent IEEE evaluator; before algebraic simplification | Implemented for non-NaN add/subtract/multiply/divide; [design](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Large** | Medium runtime and code size in numeric programs | Retains NaN-producing operations while preserving infinities, subnormals, signed zero, host independence, and compile-time cost |
| FMV-07 | Exact constant integer-to-floating conversion | With the IEEE evaluator; before floating arithmetic folding | Implemented for `i64`, `u64`, `u8`, and `bool`; [design](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md) / **Medium to large** | Low to medium runtime | Preserves source signedness and exact target bits without inheriting host or target quirks |
...
| FMC-04 | [Fold constant checked `f64`-to-integer conversions](../compiler/PHASES_AND_IR.md#target-independent-binary64-evaluation) | After exact primitive and checked-integer folding; before repeated primitive/dead/CFG cleanup | Implemented ([driver](../compiler/DRIVER_AND_ARTIFACTS.md#final-mir-optimization-selection), [reporting](../compiler/REPORTING.md#checked-floating-to-integer-protocol-simplification-observation), [testing](../development/TESTING.md#checked-floating-to-integer-protocol-simplification-coverage), [design](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_DESIGN_PROPOSAL.md), [roadmap](../archive/TARGET_INDEPENDENT_BINARY64_EVALUATION_ROADMAP.md)) / **Large** | Low to medium runtime and size | The pass atomically rewrites only fully proven successful `i64`/`u64`/`u8` diamonds, preserves source evaluation and result storage/lifetimes, and retains NaN, infinity, finite range failures, malformed, protected, and insufficiently proven protocols |
...
| FMC-11 | [Constant-left short-circuit logical CFG simplification](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_DESIGN_PROPOSAL.md) | At a narrow proof-consuming transition composed atomically with mandatory proof normalization; before final CFG cleanup | Implemented; exact planning, atomic transition composition, production registration, public selection, stable metrics, native evidence, and closure hardening are complete ([delivery](../archive/CONVERGENT_LOCAL_CONSTANT_PROPAGATION_ROADMAP.md)); broader identities remain follow-up / **Large** | Medium runtime and code size, plus high completeness value for boolean constant propagation | Preserves left evaluation; skips an unselected RHS without requiring purity; executes a selected RHS exactly once; retains selected failures/effects; avoids persistent logical provenance; keeps mandatory normalization profile-independent |
...
remain unimplemented. The frozen phase architecture design's
...
proposed [complete lowering migration design](COMPLETE_LOW_LEVEL_LOWERING_MIGRATION_DESIGN_PROPOSAL.md).
...
| [Normalization-stable path-activation provenance](../archive/NORMALIZATION_STABLE_PATH_ACTIVATION_PROVENANCE_DESIGN_PROPOSAL.md) | FMM-13 and future final-stage storage transformations | **Completed (medium)** — stable role, narrow verifier authority, consumer classification, final-CFG mutation barrier, target/observation parity, source/profile/malformed evidence, ownership, and documentation are complete in the [archived roadmap](../archive/NORMALIZATION_STABLE_PATH_ACTIVATION_PROVENANCE_ROADMAP.md) | Medium now; high before storage mutation | Implemented one final-only activation storage kind and restored normalized checking for ordinary scalar spills without retaining consumed path identities |
...
did not select the former narrow FMV-15 candidate, FMV-02, or FMV-03 on
measured corpus benefit. Subsequent review identified arbitrary-depth
constant propagation as an expected completeness property and promoted the
broader FMV-16 design on that architectural basis. A later scope review
selected the closed integer-only part of FMV-02 for
[implementation](../archive/INTEGER_CAST_CHAIN_CANONICALIZATION_ROADMAP.md): arbitrary-
length same-block chains over `u8`, `i64`, and `u64` have a complete finite
algebra and canonical recipes of at most two casts. The algebra, immutable
same-block analysis, guarded rewrite, and one independently selectable
default-profile occurrence now exist. This is an
