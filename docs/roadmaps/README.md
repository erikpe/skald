...
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Draft overarching proposal; first child drafted | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Review the phase architecture and backend ownership proposal | Verified final MIR; existing backend/runtime contracts; AArch64 and future placement requirements |
| [Low-level phase architecture and backend ownership](LOW_LEVEL_PHASE_ARCHITECTURE_DESIGN_PROPOSAL.md) | Draft design proposal | Settle layout-specialized shared LIR, phase products, lifecycle/target ownership, and foundation validation | Review the design, then create its implementation roadmap | Low-level compiler architecture direction; detailed LIR and target realization designs follow |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; low-level architecture has a draft overarching proposal | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Review the low-level architecture direction; use evidence to select other prerequisite work | Existing pipeline, reachability, normalization, and MIR identity foundations |
