...
| --- | --- | --- | --- | --- |
| [Target selection, checked placement and physical realization](TARGET_SELECTION_PHYSICAL_REALIZATION_ROADMAP.md) | In progress; complete pilot selection, baseline placement and independent checking implemented | Deliver the private whole-program native pilot through independently checked native phases | NP13 — Symbolic frame planning and limits | Frozen target design, accepted model amendment and completed-model handoff; full migration, adoption and allocation remain separate |
...
| [Low-level compiler architecture discoveries](LOW_LEVEL_COMPILER_ARCHITECTURE_DISCOVERIES.md) | One independent follow-up; one frame-layout prerequisite; signature-boundary ABI shapes resolved | Record maintenance and contract findings encountered during phase-model implementation | Resolve target ABI-area layout authority before frame objects; isolate golden build artifacts independently | Existing golden planner/compiler/native execution owners; independent of model publication |
