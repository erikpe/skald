...
| --- | --- | --- | --- | --- |
No implementation roadmap is currently active. The complete lowering migration
design is proposed for review before promotion into an implementation roadmap.

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; preparation, executable LIR model and private native pilot complete | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Review and freeze the focused complete-lowering-migration design | Verified private pilot and archived target design; full migration, adoption and allocation remain separate |
| [Complete low-level lowering migration](COMPLETE_LOW_LEVEL_LOWERING_MIGRATION_DESIGN_PROPOSAL.md) | Proposed for review | Migrate the complete supported x86 language, lifecycle, helper, data and artifact surface through the checked low-level pipeline | Confirm the retained-inactive static, owner-renaming and observation boundaries, then freeze and create the roadmap | Completed LA01–LA03 designs and implementations; maintained migration coverage record |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; phase preparation, executable LIR model and private native pilot complete | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Review complete lowering migration design; retain separate foundation adoption evidence | Existing pipeline, reachability, normalization, MIR identity and checked native-pilot foundations |
