...
| --- | --- | --- | --- | --- |
| [Low-level compiler consolidation and adoption](LOW_LEVEL_COMPILER_ADOPTION_ROADMAP.md) | Planned | Adopt one checked production pipeline, expose requested observations, accept foundation cost, retire direct lowering, and close the cumulative foundation review | AD01 — freeze adoption inventory and comparison authority | Accepted [frozen design](../archive/LOW_LEVEL_COMPILER_ADOPTION_DESIGN_PROPOSAL.md), complete private parity, and scalable checked baseline placement |

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; LA01–LA04 and placement scalability complete; LA05 planned | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Implement the active [LA05 roadmap](LOW_LEVEL_COMPILER_ADOPTION_ROADMAP.md), beginning with AD01 | Accepted [LA05 design](../archive/LOW_LEVEL_COMPILER_ADOPTION_DESIGN_PROPOSAL.md); allocation remains separate |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; complete private LIR pipeline delivered | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Preserve separate foundation cost evidence through the active LA05 roadmap | Existing pipeline, reachability, normalization, MIR identity and checked native foundations |
