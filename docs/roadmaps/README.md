...
| --- | --- | --- | --- | --- |
| [Low-level IR model, construction and verification](LOW_LEVEL_IR_MODEL_ROADMAP.md) | In progress; LI01–LI10 complete | Deliver checked lowered/selected models, builders, verification/publication, edits and immutable inspection | LI11 — Cumulative review, cleanup, downstream handoff and closure | Frozen LA02 model and archived phase design; common declaration readiness passed; selected publication readiness passed; concrete native agreements remain LA03 |

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; phase preparation complete; LIR model design frozen | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Implement the LIR model roadmap; prepare the coordinated LA03 target design | Verified final MIR; archived frozen phase design; existing backend/runtime contracts; AArch64 and future placement requirements |
| [Low-level IR model, construction and verification](LOW_LEVEL_IR_MODEL_DESIGN_PROPOSAL.md) | Accepted, frozen and promoted LA02 design | Specify lowered execution, identities/storage, CFG/guards, effects/calls, selected structural contracts, seals/editing and inspection | Complete LI11 cumulative review and closure; amend the owning design explicitly if a witness cannot be represented | Frozen phase design and preparation handoff; concrete target ABI/resource/inventory/frame decisions remain LA03 |
| [Low-level compiler migration coverage](LOW_LEVEL_COMPILER_MIGRATION_COVERAGE.md) | Preparation/declaration readiness and lowered drafts complete; executable pipeline delivery pending | Maintain operation/helper coverage, phase authority, witnesses and baseline handoff through foundation adoption | Record remaining model delivery/readiness as the roadmap progresses; preserve pending native counterpart obligations | Frozen model and archived phase design; qualified current-boundary witnesses and durable baseline; adoption cost clearance remains pending |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; phase preparation complete; LIR model roadmap in progress | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Implement the frozen LIR model contracts and coordinate the LA03 target design; use evidence to select other prerequisite work | Existing pipeline, reachability, normalization, and MIR identity foundations |
