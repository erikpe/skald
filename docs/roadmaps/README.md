...
| --- | --- | --- | --- | --- |
| [Low-level phase architecture and backend ownership](LOW_LEVEL_PHASE_ARCHITECTURE_ROADMAP.md) | Planned | Establish migration coverage, existing-boundary regressions, and qualified pre-migration measurements | LP01 — Migration contract and coverage inventory | Frozen phase architecture design; detailed LIR and target implementation remain later workstreams |

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; first child roadmap planned | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Execute the phase architecture roadmap; prepare detailed LIR design from its handoff | Verified final MIR; existing backend/runtime contracts; AArch64 and future placement requirements |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; phase architecture roadmap planned | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Execute phase architecture preparation; use evidence to select other prerequisite work | Existing pipeline, reachability, normalization, and MIR identity foundations |
