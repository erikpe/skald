...
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Draft overarching proposal | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Review the overarching direction, then write the phase architecture and backend ownership proposal | Verified final MIR; existing backend/runtime contracts; AArch64 and future placement requirements |
