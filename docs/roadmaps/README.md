...
- [Compositional Optional Types](COMPOSITIONAL_OPTIONAL_TYPES_ROADMAP.md) —
  **in progress**. Makes `(shared T)?` canonical with `shared? T` as an exact
  ...
  Shared boxes containing optionals remain a separate future roadmap. CO5 has
  completed recursive owning lifecycle; CO6, nested access and callable
  integration, is next. The work builds on
