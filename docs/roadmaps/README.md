...
No active implementation roadmap. Low-level phase preparation is complete;
the LIR model proposal awaits review with coordinated target interfaces.

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; phase preparation complete; LIR model proposal drafted | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Review the LIR proposal and joint target interfaces before promotion/roadmap creation | Verified final MIR; archived frozen phase design; existing backend/runtime contracts; AArch64 and future placement requirements |
| [Low-level IR model, construction and verification](LOW_LEVEL_IR_MODEL_DESIGN_PROPOSAL.md) | Draft LA02 design | Specify lowered execution, identities/storage, CFG/guards, effects/calls, selected structural contracts, seals/editing and inspection | Review joint LA03 counterparts, then accept/freeze and create a focused implementation roadmap | Frozen phase design and preparation handoff; target ABI/resource/inventory/frame counterpart review |
| [Low-level compiler migration coverage](LOW_LEVEL_COMPILER_MIGRATION_COVERAGE.md) | Preparation complete; all new-pipeline delivery pending | Maintain operation/helper coverage, phase authority, witnesses and baseline handoff through foundation adoption | Carry delivery and joint LIR/target questions into the next focused designs | Archived frozen phase design; qualified current-boundary witnesses and durable baseline; adoption cost clearance remains pending |
...
| [Optimization architecture discoveries](OPTIMIZATION_ARCHITECTURE_DISCOVERIES.md) | Three constraints remain; phase architecture preparation complete | Record architectural limits on modular target-independent and target-specific optimization and their sequencing | Review the draft LIR design with coordinated target interfaces; use evidence to select other prerequisite work | Existing pipeline, reachability, normalization, and MIR identity foundations |
