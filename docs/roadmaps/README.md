...
| --- | --- | --- | --- | --- |
| [Complete low-level lowering migration](COMPLETE_LOW_LEVEL_LOWERING_MIGRATION_ROADMAP.md) | Active; LM01–LM08 complete | Expand the checked private native pilot to the complete supported language, helper, data and artifact surface | Checkpoint the aggregate and lifecycle core | Accepted frozen LA04 design; completed LA01–LA03 foundations and durable migration ownership; maintained migration coverage record |

## Pending discovery and planning records

| Record | Status | Purpose | Next step | Dependencies |
| --- | --- | --- | --- | --- |
| [Low-level compiler architecture](LOW_LEVEL_COMPILER_ARCHITECTURE_DESIGN_PROPOSAL.md) | Accepted direction; LA04 implementation active | Establish explicit LIR phases and shared/target backend ownership, then implement register allocation as a separate final workstream | Reconcile the aggregate/lifecycle checkpoint before optional lowering | Verified private pilot, accepted LA04 design and maintained coverage; adoption and allocation remain separate |
