...
Status: complete and archived, 2026-09-19; NP01–NP19 complete.
...
- Preserve the [phase contract](LOW_LEVEL_PHASE_ARCHITECTURE_DESIGN_PROPOSAL.md),
  [model contract and accepted streaming amendment](LOW_LEVEL_IR_MODEL_DESIGN_PROPOSAL.md#accepted-streaming-publication-amendment),
  [completed-model evidence](../roadmaps/LOW_LEVEL_COMPILER_MIGRATION_COVERAGE.md#common-model-readiness)
  and [native obligations](../roadmaps/LOW_LEVEL_COMPILER_MIGRATION_COVERAGE.md#target-implementation-obligations).
...
- [x] NP17 — Whole-program native pilot
- [x] NP18 — Native hardening, observations and handoff
- [x] NP19 — Cumulative review, cleanup and closure
...
- [x] Implement physical drafts with concrete registers, offsets, immediates, blocks, relocations, calls and typed dependencies; retain exact selected/placement/frame provenance.
- [x] Expand transfers and bounded declared recipes, prologue/epilogue, saves and ABI movements. Require CheckedPlacement and checked frame plans; no virtual operands, symbolic offsets or assembly-string proof.
- [x] Keep target-owned instruction formatting reusable only as a typed leaf. Do not expose a final assembly entry until physical verification and program closure exist.
...
- [x] Implement exhaustive opcode/register/width/immediate/displacement checks, closure of symbolic operands and relocations, declared recipe/provenance and dependency checks.
- [x] Check CFG stack state, call alignment, saves/restores, return state, loops/joins and scratch footprints; linearly balanced stack adjustments are not sufficient.
- [x] Publish VerifiedPhysicalCallable and exact derivation receipts only from this checker. Add deterministic immutable physical inspection and requested-only frame/placement observations.
...
- [x] Add the private explicit pilot entry and owner integration harness requiring whole-program admission; ordinary public/default emission remains the legacy path with no pilot fallback.
- [x] Run admitted source programs through projection, discovery, both construction passes, selection, checking, frame/physical publication and final artifact closure. Preserve entry protocol and runtime ABI.
- [x] Add independent native phase/C probes for hidden result, receiver and alias-origin component pressure, without falsely claiming excluded aggregate source migration.
...
- [x] Complete adversarial numerical/ABI/trace/frame tests through the verified path; cover NaN predicates, unsigned float casts, minimum overflow/floor semantics, full-count shifts, live ties and caller-clobber result preservation.
- [x] Validate requested-only immutable dumps/checkpoints and quiet mode without fabricating public phase events. Keep target-independent contracts free of x86 frame/register assumptions with the synthetic target.
- [x] Update coverage with exact native witnesses and dimensions, remaining full-language/helper/static gaps, consumed allowance disposition and remaining removal owners. Record exploratory pilot cost evidence only; retain all foundation timing qualifications.
...
- [x] Review child baseline..HEAD with stat/name-status/full diff plus staged, unstaged and untracked changes. Inspect relevant foundation history and current owners; distinguish unrelated intervening changes. Manual task commits do not remove cleanup obligations.
- [x] Reconcile every artifact ledger entry against source/history, including inherited allowances, bridging APIs, draft consumers, extracted adapters, fragment experiments and private pilot gates. Remove expired artifacts; transfer necessary continuing items with exact file/symbol and owner.
- [x] Fix small ownership/interface/diagnostic inconsistencies. Resolve substantial design/correctness gaps in explicit tasks before closure; record independent opportunities in the indexed architecture discoveries.
- [x] After fixups, run artifact-free full validation and supported-toolchain/native gates. Record reviewed baseline/endpoint, residual closing changes awaiting user commit, artifact disposition and results.
- [x] Mark completion only after all exit criteria pass; archive this roadmap and the frozen target design, repair links/indexes and update parent, coverage and audit readiness. Keep full migration/adoption/allocation pending; leave committing to the user.
...
| --- | --- | --- | --- |
| `backend/effects.rs`: `MemoryRegion`, `Effect`, `Effects` and their checked set/remapping methods | `68dec8ed` | Full migration and production adoption; conservative barriers remain durable | The private pilot consumes these contracts. Remaining scoped allowances cover model surfaces not yet reached by the admitted subset and transfer to LA04/LA05 |
| `backend/plan/{facts,identities,check,view,services}.rs`, `plan/mod.rs`: `PlanFacts`, `CheckedPlan`, `PlanView`, typed IDs/declarations, checked services and explicit re-export groups | `ff12421d`, services `68dec8ed` | First native planning/lowering consumer; remaining full-surface facts retire their allowances with complete migration; reviewed at closure | Scoped allowances remain until native consumption; durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/graph/{arena,edit}.rs`, `graph/verify/{model,check,analysis}.rs`, `graph/mod.rs`: `OwnedArena`, `IdMap`, `GraphView`, `check_graph`, `GraphSession` and explicit re-export groups | `ff12421d`, verification `eecdb4cc`, edits `03e4ae42` | First native graph/analysis/edit consumers; shared algorithm stays durable; reviewed at closure | `check_graph` allowance retired NP04; remaining analysis/edit allowances stay until their consumers. Durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/lir/{model,scalar,builder,schema,read,call,trace,observable}.rs`, `lir/graph/{storage,operands}.rs`: draft records, `DraftBuilder`, `DraftChecks` and graph adapter impls | `43df9ce6`, effects `68dec8ed`, graph adapters `eecdb4cc` | First native lowered construction/checking consumers; remaining vocabulary with complete migration; reviewed at closure | `DraftBuilder` allowances retired NP04; division/shift/conversion descriptor allowances retired NP05. `Conversion::PointerBits` stays narrowly scoped until LA04 full migration; other vocabulary allowances stay until their consumers. Call construction/checker and trace construction/checker allowances retired NP06. Durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/lir/verify/{check,domains,memory,trace,lift,failure,publication}.rs`: full checking helpers, structured failures, `VerifiedCallable` and `CompletionReceipt` | `e87fa392` | First native lowered verification/publication consumer; no replacement with builder trust; reviewed at closure | Callable checking and publication/receipt allowances retired NP04; numeric guard-checker allowances retired NP05; trace checker allowances retired NP06. Callable analysis remains scoped until its consumer. Durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/lir/program/{inventory,data,target}.rs`: `ProgramBuilder`, `VerifiedProgram`, data validation, `TargetDeclarations`, `TargetCatalog` | `ddc5a97d`; catalog migrated NP01 `2cbd7ffd` | First native inventory/discovery consumer; preserve streaming receipts, plan freeze and exact parent reconciliation; reviewed at closure | Worklist begin/completion/state allowances retired NP04; constructor, data construction/validation and final closure allowances retired NP06. Other catalog/program allowances remain pending their consumers. Audit all annotations in these files, not only principal symbols |
| `backend/selected/{storage,context,builder,graph,abi,resources,description}.rs`: `SelectedDraft`, `SelectionContext`, `SelectedBuilder`, `Payload`, ABI/resource records and descriptions | `63291d6d` | Real target schema/selection consumer; retain immutable opcode-derived descriptions; reviewed at closure | Scoped allowances remain until native consumption; durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/selected/verify/{check,descriptors,failure,publication,program}.rs`: shared checking helpers, `TargetVerifier`, `VerifiedSelectedCallable`, `SelectedReceipt`, `SelectedProgramBuilder` | `73b1fafe`; closure migrated NP01 `2cbd7ffd` | First native selected verification consumer; both shared and target checks remain mandatory; reviewed at closure | Scoped allowances remain until native consumption; durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/{lir,selected}/edit/{mod,editor,rebuild}.rs`, `lir/edit/split.rs`: `LoweredEditor`, `SelectedEditor`, `LoweredRemap`, `SelectedRemap`, remapping/rebuilding and split helpers | `03e4ae42` | Native edits/analysis consumers; retain consuming authority and full reverification; reviewed at closure | Scoped allowances remain until native consumption; durable APIs remain. Audit all annotations in these files, not only principal symbols |
| `backend/{inspection,lir/inspect,selected/inspect}.rs`, `lir/mod.rs`, `selected/mod.rs`: visitors/renderers, immutable enumeration and explicit facade re-export groups | `495df6b9` | NP18 private checkpoint consumer; LA05 public adapters and requested metrics; reviewed at closure | NP18 consumes verified callable/program dumps and removes their non-test allowances. Visitor-only allowances remain narrow until a production adapter consumes them. No fabricated observation or mutable inspection product |
| `backend/x86_64_sysv/native/{mod,resources,abi}.rs`: `Gpr`, `NativeResources`, `ComponentAbi`, classifier, explicit facade imports and scoped non-test allowances | NP02, baseline `2cbd7ffd`; introducing commit `9a9e3bc1` | NP07/NP09 actual native selection consumers; NP12/NP13 placement/frame consumers; reviewed at closure | NP07 retired resource/classifier/type/entry allowances. NP08 retired call/noreturn and caller-clobber allowances through the numeric reporter consumer. NP12 consumes preservation; NP13 consumes outgoing extent. NP15 removes the unused REX query/allowance: high-byte registers are unrepresentable and verified leaf encoding witnesses cover every low-byte register. Tests retain lint checks; no legacy adapter or production switch |
| `backend/pilot/{mod,facts,projection}.rs`, `x86_64_sysv/pilot_facts.rs`: private admission facade, admitted getters, projection entry points and item-scoped non-test allowances | NP03, baseline `9a9e3bc1`; introducing commit `879bfb47` | NP04/NP06 lower consumers; NP17 private pipeline; LA04 full migration and LA05 public adoption | Durable checked planning boundary; program/plan/layout/signature getter allowances retired NP04; trace fact allowances retired NP06; NP17 consumes admission through the private pilot. No temporary lowerer, fallback, fake intrinsic input or production switch |
| `backend/pilot/lower/`: shared adapter and private facade allowance | NP04, baseline `879bfb47`; introducing commit `a4c3f189` | NP05 numeric lowering; NP06 calls/trace/entry and program construction; NP17 private pipeline; reviewed at closure | NP05 retired numeric pending gates. NP06 retired all remaining pending-feature variants, duplicated preflight and call/trace/entry rejection branches, and replaced the intrinsic test bypass with complete worklist closure. NP17 adds the target-neutral fallible streaming adapter and consumes it for separate exploratory/executable passes. No fake body, trap, receipt or fallback |
| `backend/pilot/lower/tests/oracle.rs`: private numeric fixture execution oracle | NP05, baseline `a4c3f189`; introducing commit `430a30bc` | Permanent owner-local tests; retained after closure review | Retain independent lowering association/boundary evidence, with unsupported operations rejected. No production interpreter, target parity claim or executable authority |
| `backend/x86_64_sysv/native/selected/{context,select}.rs`: private entry non-test allowances and explicit unsupported recipe preflight | NP07, baseline `5369ea69`; committed with NP08 as `e35be34f` | NP08 numeric/failure recipes, NP09 calls/trace; NP17 private orchestration; reviewed at closure | Retain real immutable scalar selector/verifier, no fake body or fallback. NP08 retired numeric/check/failure preflight and rejection cases; NP09 retired the call/trace and nonreturning/trap gates; NP17 consumes and removes the context/selection entry allowances. Out-of-pilot pointer/scaled-index recipes still reject |
| `backend/x86_64_sysv/native/selected/{numeric,recipes,verify/numeric}`: concrete numeric cells, domains and zero-code operation/result associations | NP08 atop uncommitted NP07, HEAD `5369ea69`; committed together as `e35be34f` | Durable target checking metadata; NP14 finite expansion, reviewed at closure | Retain markers only while independently checking actual CFG/definitions; no machine instruction, operand, hidden correction or emission authority |
| `backend/x86_64_sysv/native/selected/tests/oracle.rs`: concrete selected-cell test interpreter | NP08 atop uncommitted NP07; committed as `e35be34f` | Permanent owner-local recipe evidence, reviewed at closure | Test-only, independent of selector/verifier mappings; rejects unsupported cells; never production execution or native parity authority |
| Native `Opcode::Failure` and shared call trace-barrier clarification | NP08 numeric prerequisite; commit `e35be34f` | NP09 general call/trace consumers; NP14 declared reporter/trap expansion; reviewed at closure | Durable narrow canonical reporter terminal; retire no implementation bridge. General calls/traces are selected by NP09 |
| `SelectionContext::abi_areas`, `ObjectRole::Abi`, selected slot checks | Global shapes introduced LA02; replaced NP07, baseline `5369ea69` | Removed by NP07; absence confirmed at closure | Global shape storage and unqualified ABI objects removed; one signature registry preserves shared program authority; no compatibility adapter |
| Finalized-parent construction compatibility adapters | None introduced | Absence confirmed at closure | NP01 directly replaced the old API; no compatibility adapter or alias introduced |
| Frozen lower trace facts in `lir::CompletionReceipt`; native call fields, concrete trace cells and pure requests | NP09, task baseline `e35be34f` | NP10–NP16 placement/realization/closure; reviewed at closure | Durable narrow immutable facts survive body release; no retained executable lower body, opaque trace payload, thunk bridge or instrumentation |
| `backend/placement/{model,structure}.rs`: unchecked drafts and non-test dead-code allowances | NP10, task baseline `3413f792`; committed `1d0b84e6` | NP11 checker and NP12 producer; reviewed at closure | NP11 retired module-wide model/structure allowances and consumed draft/type allowances; NP12 retires the draft builder and home/save/transfer-scratch construction allowances. Only `Spill` construction remains narrowly scoped until a spilling producer. Structural validation grants no authority; no producer success flag or realization bypass |
| `backend/placement/check.rs`: private immutable checked queries; native `check_native_placement` entry and facade allowance | NP11, task baseline `1d0b84e6` | NP12 producer, NP13/NP14 frame/realization consumers and NP17 private orchestration; reviewed at closure | Sole checked constructor runs legality, finite convergence and strict replay. NP13 retires the checked-query impl allowance and consumes selected, assignment, storage and transfer queries. NP14 consumes the exact-selected query; only native entry allowances remain until orchestration; no unchecked constructor, mutable draft accessor or producer flag |
| `backend/placement/tests/oracle.rs`: independent finite-state specification and native resource probe | NP10, task baseline `3413f792` | Durable independent checker regression reference, reviewed at closure | Remains test-only and independent of producer/checker availability maps. No production path sampling, placement authority or temporary execution bridge |
| Exploratory `lower_program_with` pass and shared `discover_requests` rule | NP17, task baseline `e091c4ac`; committed `2d252cc3` | Durable two-pass schedule; reviewed at closure | Discovery receipts are dropped before catalog freeze and never become executable authority. Retain the pure request rule shared with selection; no instrumentation or receipt bridge |
| Test-only physical draft views/clones in `native/physical/verify/publication.rs` and adversarial fixtures in `native/physical/{tests,verify/tests}.rs` | NP14–NP15; `1e573da3`, `284146f3` | Permanent verifier tests | Retained only for malformed-draft construction and exact-parent rejection tests. Rendering and publication accept verified products only; no production shortcut |
| `native/physical/program/store.rs`: `FragmentStore`, temporary store constructors and injected-store tests | NP16; `e091c4ac`, hardened `bcf4cb6d` | Durable private program closure; LA05 adoption decides final storage integration | Retained as failure-safe typed-key bounded body storage. No serializer, importer or cache; successful and failed closure remove owned fragments, and stale path collisions are skipped |
| `backend/x86_64_sysv/native/pilot::{compile_native_pilot,compile_native_pilot_inspected,NativePilotInspection,NativePilotError}`: private explicit pilot and requested observation entries with scoped non-test allowances | NP17, task baseline `e091c4ac`; committed `2d252cc3`; observation added NP18 | LA05 adoption/public observation adapter; LA05 production adoption or removal | Quiet compilation never constructs output. Requested verified checkpoints stream to a supplied writer and writer failure prevents publication. Ordinary emission has no branch to the pilot and errors have no fallback. Retire the allowances when production adopts or removes the pilot |
| Synthetic resource/frame fixtures and manual register placements under `backend/{selected,placement,frame}/tests` | NP02/NP10 onward; `9a9e3bc1`, `1d0b84e6`, `9e6e2405`, `1d248659`, `8d842643` | Permanent owner-local tests | Retained as independent portability and checker witnesses; no production target registration, exports or blanket test-only phase gating |
| `backend/placement/baseline/{produce,operands}.rs`, `placement/transfers.rs`: descriptor-local baseline and shared deterministic copy resolver | NP12, task baseline `9e6e2405`; committed `1d248659` | Durable producer/resolver; NP13 frame requirements, NP14 realization, NP17 orchestration; reviewed at closure | No producer success flag, availability proof map, fake checked product or compatibility bridge. Typed point-local cycle storage and target-declared working views remain explicit. NP17 consumes and removes the native baseline entry allowance |
| `backend/frame/{model,layout,planning,addressing}.rs`, native `plan_native_frame`: immutable exact-placement frame plans and item-scoped consumer-query allowances | NP13, task baseline `1d248659`; committed `8d842643` | Durable layout authority; NP14 query consumers, NP17 native orchestration; reviewed at closure | No frame draft, unchecked receipt, late storage/scratch allocation or legacy adapter. NP14 retires consumed policy/object/address/size/exact-placement query allowances; NP15 consumes storage/outgoing queries for requested frame inspection; NP17 consumes and removes the native frame entry allowance. Synthetic link-role construction remains item-scoped until a real second target |
| `backend/selected/area_layout.rs`, `SelectionContext::with_abi_layout`, native canonical area checks | NP13, task baseline `1d248659`; committed `8d842643` | Durable signature-local layout authority; reviewed at closure | Replaces representation-width summation directly. Shared shape/layout checks and independent target canonical checks remain separate; no x86 stride in shared validation |
| `backend/x86_64_sysv/native/physical/{model,realize,operands,recipes,transfers,format}.rs`: private typed drafts, exact-input realizer, target instruction leaf and scoped non-test field/entry allowances | NP14, task baseline `8d842643`; committed `1e573da3` | Durable concrete target vocabulary; NP15 independent checker, NP16 verified fragment formatting, NP17 orchestration; reviewed at closure | Drafts retain exact frame/placement/selected borrows. NP15 removes draft-field/instruction-leaf allowances; NP17 consumes and removes realizer and physical-check entry allowances. No unchecked publication seal, final emitter, legacy adapter or semantic-operation escape |
| `native/physical/tests.rs`: draft-to-assembler encoding witness | NP14, task baseline `8d842643`; committed `1e573da3` | NP15 verified-product tests; reviewed at closure | NP15 reroutes assembler witnesses through independently checked physical publication. Test-only draft views/clones remain solely for adversarial checker fixtures. Retain instruction-leaf encoding coverage; assembler acceptance is never execution, state or closure authority |
| `native/physical/verify/{encoding,authority,recipes,derivation,state,publication}.rs`: independent acceptance, immutable publication, exact parent receipts and requested inspection | NP15, task baseline `1e573da3`; committed `284146f3` | Durable physical authority; LA04 expands supported inputs and LA05 adopts observation | Sole consuming constructor requires exact checked inputs. NP16–NP18 consume receipts, verified bodies and physical/placement/frame rendering. Test-only visitor/parent helpers and clones/body views remain narrowly scoped for adversarial fixtures |
| `native/physical/program/{mod,render,store}.rs`: exact program reconciliation, checked rendering and private temporary fragments | NP16, task baseline `284146f3`; committed `e091c4ac` | Durable final physical authority and bounded body storage; LA05 production adoption | Only verified physical callables render. NP17 consumes closure/publication entries and preserves runtime-owned symbols. NP18 hardens temporary creation against stale PID/counter paths. Test-only injected stores cover failures; production fragments are deleted on completion/drop |
...
### Cumulative closure record

NP19 reviewed implementation baseline `f150d028` through committed endpoint
`bcf4cb6d`. The range contains the NP01–NP18 task commits and no unrelated
intervening commit: 188 files changed, with 27,220 insertions and 466 deletions.
The review covered the stat, name-status and full cumulative diff, every task
commit, the clean endpoint tree and all closing changes. The residual uncommitted
change is this documentation/ownership closure: archive moves, repaired links and
indexes, current parent/coverage/audit handoffs, living-document status fixes and
removal of roadmap task labels from production-facing documentation and comments.
It changes no compiler behavior; the user owns the closing commit.

Every ledger row was checked against source and history. No finalized-parent
compatibility adapter, unchecked physical renderer, discovery receipt bridge,
fallback, fake body, serializer/importer/cache or public production switch
remains. Streaming catalogs and receipts, the two-pass discovery schedule,
independent selected/placement/physical verification, checked baseline placement,
symbolic frames and failure-safe typed fragment storage are durable architecture.
Synthetic target/resource fixtures, manual register placements, independent
oracles and malformed-draft views remain permanent owner-local tests. Scoped
model-surface and unsupported-language allowances transfer to LA04; the private
pilot, requested observations and production storage integration transfer to
LA05; `Spill` construction and allocation policy transfer to LA06. The existing
concurrent golden-artifact discovery remains independently owned. Review found no
new correctness gap or independent follow-up.

Serial artifact-free validation passed `make check`, including formatting,
locked all-target checks, Clippy with warnings denied, valid documentation,
3,454 compiler tests, all workspace/runtime suites and 650 golden cases.
`make msrv-check` passed with Rust 1.82.0. `make check-long` passed full golden
determinism, release golden execution, the 10,000-case generative robustness run,
runtime-trace measurement, the generic-vector probe and all range-loop targets.
Final whitespace, link, stale-reference, cumulative-diff and working-tree checks
found only the intended closing files. The frozen design and completed roadmap
are archived; the maintained migration coverage now hands complete lowering to
LA04 while production adoption and allocation remain pending in LA05 and LA06.
...
## NP14 implementation record

Task baseline: `8d842643`, the user's committed NP13 frame-planning change.
Inspected the committed frame and placement interfaces and their consumer
allowances before adding realization. No reset or committed scaffolding removal
was needed.

The target-owned physical draft vocabulary contains concrete register identities,
widths, immediates, signed displacements, local block identities and typed
relocation/call artifacts. Its immutable frame borrow retains the exact selected
and checked-placement authorities; equal-content replacements reject. Ordered
groups retain selected sites, transfer occurrences, frame actions and successor
occurrences, including zero-code metadata obligations.

Realization consumes existing checked locations and frame coordinates, expands
finite selected recipes within their declared bounds and emits the fixed SysV
entry/return protocol. Explicit transfers supply ABI movements, preservation
and parallel-copy scratch. Edge transfers receive administrative forwarders;
semantic numeric correction CFG is unchanged. Native unsupported frame policies
and sizes reject before expansion. No late storage, semantic correction,
helper discovery or program publication was added.

The reusable target formatter accepts individual typed instructions only.
The runtime-attribution construction audit explicitly recognizes the two physical
call owners (finite recipes and leaf matching); legacy call allowances remain
unchanged and no physical owner invokes the legacy attribution facade.
Draft assembler tests witness encoding acceptance, not execution or publication;
the ledger assigns their verified-product transition to NP15. Owner tests cover
scalar and numeric cells, calls and indirect targets, tracing enabled/omitted,
pressure, forwarded copies, exact provenance, bounded steps, all primitive cast pairs, unsigned/byte arithmetic and all GPR/XMM
views. Existing manually checked register, callee-save and memory-copy placements
now also feed the realizer. The byte-carrier review confirmed that native
selection intentionally requires scalar-width accesses; speculative wider-store
scratch expansion was removed rather than retained without a checked producer.

Validation: all five physical-owner tests passed, including assembler witnesses.
Final-source `make check` passed: 3,435 compiler tests, all workspace/runtime
suites and 650 golden cases, with zero failures. Serial `make msrv-check` passed
with Rust 1.82.0. Formatting, Clippy, documentation links/indexes and whitespace
checks passed. The first full run exposed the legacy raw-call audit mismatch;
the corrected complete run passed. No independent discovery was added.

NP14 is complete; NP15 is next. Independent physical verification, native
execution parity and program closure remain later obligations. No final renderer
or default-path switch is exposed. Changes remain uncommitted for the user.
## NP15 implementation record

Task baseline: `1e573da3`, the user's committed NP14 physical realization.
Reviewed exact checked-input borrows and the artifact ledger; committed status
was not used as evidence that draft consumers were already removed.

Separate target-owned modules check encoding legality, exact typed dependencies,
selected/transfer/frame recipe derivation and reachable CFG frame state. A finite
acceptance cursor consumes actual instructions against immutable authorities;
it never invokes the realizer, its location decoder or assembly parsing. Group
and successor provenance account for the whole body, with an indexed selected-site
lookup rather than repeated full-block searches. Unsupported widths, references,
scratch, recipe steps and administrative dependencies reject.

Stack state tracks SP/FP/header provenance and original promised register bits.
A checked finite work bound covers intersection at joins/loops, followed by strict
return preservation replay. Partial saves/writes lose original-bit evidence.
Legal bitwise transfers retain those bits through XMM registers; calls destroy
all SysV XMM evidence. A manually checked placement exercises the cross-bank
round trip, and a separate state regression rejects carrying it across a call.
Returning paths restore the incoming FP/SP and untouched return address; calls
must be aligned. Nonreturning paths end explicitly, without invented epilogues.

Only the independent constructor publishes immutable physical products and
body-free exact-selected-parent receipts with unique physical snapshot identity.
Deterministic immutable text/visitor inspection is implemented, with placement
and frame regions collected only on explicit request. Assembler witnesses now
require verified products; test-only draft views/clones remain for corruption.
Native manual register, memory-copy and callee-save fixtures use the same checker.
The unused REX query is removed; closed low-byte identities and assembler view
coverage replace the transitional query allowance. Living documentation and the
native handoff describe the checked boundary without claiming execution parity.

Validation: final-source `make check` passed with 3,442 compiler tests, all
workspace/runtime suites and 650 golden cases. All 12 physical-owner tests passed
in debug and release builds. Serial `make msrv-check` passed with Rust 1.82.0.
Formatting, Clippy, documentation links/indexes and whitespace checks passed.
The first full run exposed one missing finite encoding-check owner in the legacy
textual call audit; its explicit owner list has been corrected. No independent
...
## NP17 implementation record

Task baseline: `e091c4ac`, the user's committed NP16 program-closure change.
The committed two-pass amendment and artifact ledger were reviewed before
implementation; the clean starting tree was not treated as proof that earlier
scaffolding had already been retired.

The private x86 pilot first runs exploratory verified lowering and collects the
same pure typed requests that executable selection checks. It discards those
receipts, freezes the plan-bound target catalog, then performs a fresh executable
lowering pass. Each actual body passes selected publication, selected-program
receipt registration, baseline placement, frame planning, typed realization,
independent physical verification and temporary fragment publication before its
resident predecessors are released. Final lower, selected and physical closure
reconciles both exact parent inventories before immutable assembly is returned.
Every phase error remains explicit; the pilot has no legacy fallback and no
public target-registry route.

Execution exposed and corrected two inherited program-rendering ABI defects:
the generated entry now calls the frozen `ska_rt_abi_v9` marker spelling, and
trace relocations name runtime-owned `ska_rt_trace_top` without defining private
or duplicate TLS storage. Source execution covers primitive integer/byte/boolean/
float values, local memory, loops, casts, guarded division/failure, direct and
secured indirect calls. An explicit eight-cell matrix crosses default/minimal MIR
schedules, enabled/omitted tracing and complete/reachable artifact policies.
Scalar C probes call in both directions and cross seven-integer/nine-floating
argument pressure. Existing independent native fixtures remain the authority for
hidden destinations and receiver/alias-origin component triples; those excluded
aggregate source forms are not claimed as migrated. A rejection regression also
proves the unchanged public backend still compiles and executes such source.

The reusable test support now supplies the actual minimum-optimization MIR
pipeline and real-runtime C probe linkage. Consumed admission, selection,
placement, frame and physical entry allowances were removed; the only new scoped
allowance is the explicit private pilot gate and error until LA05 adopts or
removes it. All five focused pilot tests pass in debug and release builds. Final
serial `make check` passed formatting, locked all-target checks, Clippy with
warnings denied, 3,449 compiler tests, the workspace/doc/runtime suites and 650
golden cases. Serial `make msrv-check` passed with Rust 1.82.0. NP17 is complete;
NP18 is next.

## NP18 implementation record

Task baseline: `2d252cc3`, the user's committed NP17 whole-program pilot.
The committed two-pass schedule and private-gate ledger were reviewed before
hardening; no committed predecessor work was mistaken for absent scaffolding.

The private pilot now accepts explicit, independent lowered, selected, physical,
placement and frame inspection requests. Each checkpoint is rendered by its
verified phase owner while that immutable product is live. Quiet compilation
passes no writer and performs no rendering; output failure aborts before final
program publication. Physical code became an explicit request alongside the
existing placement and frame sections. Deterministic same-process and child-
process tests cover the complete checkpoint stream and final assembly without
introducing public phase events, CLI behavior or a production-path branch.
The cross-process probe exposed a stale PID/counter temporary-directory collision;
fragment storage now skips occupied candidates while retaining exclusive creation.

Native hardening executes unordered NaN predicates, both upper-half unsigned
float conversion directions, signed minimum overflow behavior, negative floor
division/remainder and full original shift counts through the verified path.
A destructive shift tie remains live with its input, and another result survives
seven-integer caller pressure; the C boundary also crosses nine floating
arguments and demonstrates nonzero outgoing frame storage. The original
default/minimal, enabled/omitted trace and complete/reachable matrix now observes
selected/physical checkpoints in representative cells and proves trace omission
is isolated. Existing synthetic resource/frame fixtures continue to exercise
overlapping widths, partial preservation and link-role policy without embedding
x86 names in shared contracts.

The migration coverage record now names the exact pilot witnesses and transfers
the remaining work: LA04 owns excluded lifecycle, aggregate, helper, dispatch,
I/O and static surfaces; LA05 owns public observations, pilot adoption/removal,
legacy removal and foundation cost clearance; LA06 owns allocation. A small
all-checkpoint fixture produced 217,376 bytes of checkpoint text and 5,476 bytes
of assembly. This is exploratory structural evidence for LA05's presentation
design, not timing or adoption evidence. The preserved foundation baseline and
all eleven inconclusive compile timings remain qualified and unchanged.

Verified callable/program dump allowances are retired by the pilot consumer.
Visitor-only and receipt-parent helpers remain narrowly test-only; the private
pilot/observation entries retain scoped allowances until LA05 adopts or removes
them. No compatibility adapter, public event, timing instrumentation or separate
discovery was introduced. All nine focused pilot tests pass in debug and release;
all 71 native-owner tests pass. Final serial `make check` passed formatting,
locked all-target checks, Clippy with warnings denied, documentation, 3,454
compiler tests, workspace/runtime suites and 650 golden cases. Serial
`make msrv-check` passed with Rust 1.82.0. `make check-long` passed full golden
determinism, release golden execution, 10,000-case generative robustness and the
runtime-trace, generic-vector and range-loop probes. NP18 is complete; NP19 is
next. Changes remain uncommitted for the user.
