...
| **generic interface** | A frozen compile-time interface template with named type parameters. Source syntax, honest identities, definition-module signature resolution, structural type terms, bounds, and contextual requirements are implemented. Closed applications remain gated with `RES051`; each future accepted application will denote a distinct ordinary exact interface before HIR. |
