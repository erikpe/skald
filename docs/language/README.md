...
| **array** | A built-in invariant fixed-size sequence. The frozen design distinguishes deep-copying inline `T[]` values from shared `shared T[]` allocations and keeps shared projection explicit. Primitive inline values execute construction, access, deep copy, adoption, replacement, fields, and internal calls/results on x86-64; later array profiles remain behind target legality. |
