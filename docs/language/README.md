...
| **generic interface** | A frozen compile-time interface template with named type parameters. Source syntax and exact AST preservation are implemented; semantic use is explicitly gated with `RES051`. Each future accepted explicit closed application will denote a distinct ordinary exact interface before HIR. |
