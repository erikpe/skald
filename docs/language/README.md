...
| **array** | A built-in invariant fixed-size sequence. The frozen design distinguishes deep-copying inline `T[]` values from shared `shared T[]` allocations and keeps shared projection explicit. Array syntax is parsed, but resolution and execution are not yet implemented. |
