...
| **array** | A built-in invariant fixed-size sequence. The frozen design distinguishes deep-copying inline `T[]` values from shared `shared T[]` allocations and keeps shared projection explicit. Arrays are not yet implemented. |
...
- [Arrays](ARRAYS.md) freezes the not-yet-implemented inline/shared array type,
