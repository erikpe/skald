...
| **generic interface** | A compile-time interface template with named type parameters. Each valid requested closed application becomes a distinct ordinary exact interface. Class claims and generic bounds use exact nominal conformance, and bound-selected calls map template requirements to ordinary interface dispatch through verified MIR and native execution. |
...
- [Interface-based operator overloading](OPERATOR_OVERLOADING.md) defines the implemented
  ...
  Definition-site generic selection closes once to class-witness dispatch or
  an existing primitive operation, including manual bound calls.
