...
| **generic interface** | A frozen compile-time interface template with named type parameters. Source syntax, template and requirement identities, real parameter ownership, and module declaration kinds are implemented; semantic application is explicitly gated with `RES051`. Each future accepted explicit closed application will denote a distinct ordinary exact interface before HIR. |
