...
| **array** | A built-in invariant fixed-size sequence. The frozen design distinguishes deep-copying inline `T[]` values from shared `shared T[]` allocations and keeps shared projection explicit. Primitive inline local construction, immutable length, and cleanup execute on x86-64; later array profiles remain behind target legality. |
