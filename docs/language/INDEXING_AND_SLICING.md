...
classes rather than compiler language items for this feature.
...
`Str` will provide only `index_get` and `slice_get`, sharing its checked byte
lookup and constant-time descriptor slicing. Omitted slice bounds map to the
logical beginning and end. It will provide no setter and remains immutable.
