...
|---|---|---|
| [Strings](STRINGS.md) | **Partial (STR2)** | Raw-byte literals are lexed and decoded, conditionally reach the exact provider-owned `std::str::Str`, validate its private descriptor contract, and become deterministic intrinsic produced values in resolved IR, typed HIR, and verified MIR. MIR gives immutable immortal backing a distinct origin and proves exact descriptor publication plus ordinary lifecycle ownership. Provider-less use and invalid language items are diagnosed before HIR; the complete driver stops after verified MIR with a structured target-emission diagnostic. Backend emission, execution, and standard-library operations follow the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
