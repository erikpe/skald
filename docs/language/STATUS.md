...
|---|---|---|
| [Shared optional boxes](OPTIONAL_VALUES.md#shared-optional-boxes) | **Frozen design; typed local HIR** | `shared P?` is a non-null owner of an immutable published `P?` wrapper; `(shared P?)?` is its canonical optional owner and `shared? P?` is exact shorthand. Parsing, allocation recovery, exact optional targets, class/base/interface/`Obj` static views, local owner compatibility, and destination-directed construction plans are implemented through HIR. MIR ownership, native realization, access, and stored-position integration remain with later roadmap tasks. |
