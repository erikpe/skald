...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Exact-target local and callable semantics** | Exact-class shared locals initialize and assign from named owners or ordinary allocations through explicit verified copy/adopt/release/move operations. Same-target owners cross internal function, initializer, method, and interface parameters/results with caller handoff, callee cleanup, integer-class arguments, and `rax` results. The x86-64 backend executes checked allocation and retain, secure-before-release assignment, full-expression boundaries, dynamic destruction, and last-owner deallocation. Fields, polymorphic views, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
