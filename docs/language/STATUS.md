...
| [Zero-default static fields](STATIC_FIELDS.md) | **Frozen design** | Future `static name: T;` and `private static name: T;` class-owned mutable places for primitives, supported inline optionals, optional shared owners, and inline arrays. Class/module qualification, inherited canonical storage, declaring-class privacy, zero initial values, ordinary replacement, process lifetime without final cleanup, and an unchanged runtime ABI are settled. The compiler still rejects static-field declarations. |
