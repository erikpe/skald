...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Call anchors implemented** | Compatible same/ancestor/interface/`Obj` owner views cross internal calls without reallocating or replacing metadata. Shared-backed alias arguments and receivers borrow stable owners directly and anchor replaceable or produced owners through complete-call cleanup. Shared fields execute as aligned owning graph edges, and strong cycles deliberately leak. Plain shared-backed checked places and explicit copy allocation remain gated. Dynamic-type-preserving cloning is deferred. |
| [Shared object-cast extension](OBJECT_CASTS.md) | **Implemented contract** | `(shared T) source` preserves one allocation, dynamically checks when required, copies named owners, and transfers produced owners. It never allocates, slices, or copies payload. Future copy allocation through `new T(copy source)` remains planned. |
