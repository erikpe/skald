...
|---|---|---|
| [Panic and compiler-known unrecoverable failures](ERRORS.md#frozen-panic-design) | **Partially implemented** | Canonical imported `std::error::panic(Str)`, checked cast, optional, array, string-bound, and valid host-allocation failures use one length-delimited reporter. Panic is uncatchable and non-unwinding, while compiler/runtime defects remain hard traps. Ownership-overflow migration and final hardening remain. |
