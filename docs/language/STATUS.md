...
| [Integer bitwise operators](TYPES_AND_VALUES.md#implemented-integer-bitwise-operators) | **Implemented contract** | Prefix `~` and binary `&`, `|`, and `^` accept exact `i64`, `u64`, or `u8` operands, evaluate eagerly left to right, preserve fixed-width bits, canonicalize `u8`, and execute through verified MIR and native x86-64 without runtime support. |
...
| [Intrinsic declarations](MODULES_AND_INTEROP.md#intrinsic-function-declarations) | **Implemented contract** | Contextual bodyless top-level `intrinsic fn`; only the exact public `std::error::panic(message: std::str::Str) -> unit` declaration is accepted. Imports resolve by stable identity, call statements become the dedicated non-returning panic operation, and expression-position use is rejected. |
...
| Area | Maturity | Current compiler boundary |
|---|---|---|
| [Primitive operator profile](TYPES_AND_VALUES.md#frozen-primitive-operator-profile) | **Frozen design** | Exact primitive unary and binary matrices; no implicit conversion; frozen precedence with non-associative comparison/`is`; wrapping integer arithmetic; floor signed division and divisor-sign remainder; checked `u64` shift counts; IEEE binary64 division and unordered comparison; exact boolean equality and mandatory short-circuit `&&`/`||`; selected-path full-expression cleanup; and three compiler-known panic reasons. The compiler supports the source subsets listed under implemented primitive expressions, integer bitwise operators, integer comparisons, eager boolean operators, and short-circuit boolean expressions. Checked shifts additionally have a directly constructed HIR-to-verified-MIR-to-native path and exact static failure reason, but `<<` and `>>` remain unaccepted source syntax. Other frozen tokens, operations, and failure reasons remain unimplemented. |
