...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **First exact native lifetime** | The exact `shared C = new C(arguments)` local profile lowers through verified MIR and executes on x86-64 with checked runtime allocation, count-one publication, dynamic exact-class destruction, and one last-owner deallocation. Compiler-generated retain and invalid-count machinery exists for verified MIR fixtures. Broader source copies, assignment, temporaries, calls/results, fields, polymorphic views, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
