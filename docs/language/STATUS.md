...
|---|---|---|
| [Strings](../roadmaps/STRINGS_DESIGN_PROPOSAL.md) | **Exploratory direction** | The proposed design is complete: `std::str::Str` is a logically immutable inline descriptor over shared raw-byte storage, and literals use compiler-emitted immortal backing. Private fields and methods are implemented; freezing remains blocked on static methods. Strings are not accepted syntax or an executable contract. |
...
| Static state and broader class features | **Open question** | Static members, abstract/final forms, method/function overloads, reflection, and user-defined conversions are not current language contracts. Declaring-class field and instance-method privacy and ordinary initializer overloading are implemented separately above. |
