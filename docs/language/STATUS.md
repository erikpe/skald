...
|---|---|---|
| [Explicit array element-list construction](ARRAYS.md#frozen-explicit-element-list-construction) | **Frozen design; primitive execution implemented** | Typed `T[]{...}` and `new T[]{...}` determine length from an ordered list, allocate unpublished backing before element effects, initialize each exact stored element destination once from left to right, and publish only a complete prefix. Inline and shared-outer lists of `i64`, `u64`, `u8`, `f64`, and `bool` execute through verified MIR and x86-64. Type checking also selects class, optional, nested-array, shared-owner, and optional-owner plans in HIR; those lifecycle-bearing families remain behind the structured executable-lowering gate. No default construction or assignment is introduced, and runtime ABI version 8 is unchanged. |
