...
|---|---|---|
| [Strings](STRINGS.md) | **Partial (STR4)** | Raw-byte literals execute through deterministic immortal backing and ordinary descriptor lifecycle. The canonical standard-library class provides copying construction, length and checked-byte observation, `O(1)` slicing, independent array conversion, concatenation, and dynamic last-owner reclamation without compiler-selected method names. Broad hardening and promotion follow the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
