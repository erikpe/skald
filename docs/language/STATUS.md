...
|---|---|---|
| [Strings](STRINGS.md) | **Frozen design** | Exact raw-byte `std::str::Str` descriptor and language item; logically immutable shared backing; byte literal/escape semantics; conditional module dependency; compiler-emitted immortal literal storage; and ordinary standard-library operations. No string syntax or execution is implemented yet; work is ordered by the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
