...
| [Multiple files and modules](MODULES_AND_INTEROP.md#future-modules-and-broader-interoperation) | **Open question** | The design-complete proposed first whole-program system covers composable roots, `::` module qualification, explicit selective imports, top-level visibility, rejected import cycles, file or logical CLI entry selection, standard-library provider control, compatible external-ABI coalescing, compiler identity, and filesystem normalization. It remains unimplemented and awaits formal promotion to a frozen language contract. Separate compilation and package distribution remain deferred. |
