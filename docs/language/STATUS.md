...
| [Panic runtime traces](ERRORS.md#frozen-panic-runtime-traces) | **Implementation in progress** | Runtime ABI version 9 renders directly supplied newest-first chains allocation-free with a 256-row cap. The x86-64 backend now has explicit source-aware trace input plus deterministic requested-only semantic metadata, while the production driver deliberately omits it. Frame maintenance, location attribution, default-on emission, and `--omit-runtime-trace` remain pending, so generated programs retain single-line panic output. The source grammar and panic API are unchanged. |
