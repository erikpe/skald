...
|---|---|---|
| [Dynamic polymorphism](POLYMORPHISM.md) | **Partial implementation** | Static hierarchy, lifecycle, inherited access, ancestor/`Obj` views, slicing, and opt-in virtual methods execute on x86-64. Interface declarations, exact inherited conformance maps, non-owning conversions, forwarding, and calls lower into verified MIR, but interface backend execution remains unavailable. Type tests and narrowing remain unavailable. |
