...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design** | The compiler lexes and parses recursive array types, ownership grouping, construction, indexing, slicing, assignment destinations, and explicit shared bracket projection into lossless syntax nodes, then deliberately rejects them during resolution with `RES023`. No array form is typed or executable yet. Deep named copies, produced-backing adoption, default construction, jagged nesting, deterministic element lifetime, and backing anchors remain frozen for later implementation; rich initialization, inline optional array payloads, slice views, resizing, iteration, and external ABI remain deferred. |
