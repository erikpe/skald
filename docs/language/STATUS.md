...
| [Zero-default static fields](STATIC_FIELDS.md) | **Declaration frontend implemented** | `static name: T;` and `private static name: T;` parse into dedicated declarations with independent dense identity, shared ordinary-member collision rules, canonical inherited identity, declaring-class privacy, and deterministic dumps. Static expressions and runtime storage remain unavailable; `TYP042` prevents any declaration from crossing into typed HIR until the next implementation stage. The zero-state, replacement, process-lifetime, and unchanged-runtime-ABI contract remains frozen. |
