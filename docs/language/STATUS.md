...
| [Optional values](OPTIONAL_VALUES.md) | **Implemented contract** | Primitive and exact-class `T?` plus class/interface/`Obj` `(shared T)?` locals, fields, internal parameters/results, methods, interfaces, overrides, initializer overloads, conditional lifecycle, checked class payload views, secured shared unwrap, inline optional-container aliases, one-word optional-owner ABI, casts after unwrap, and anchors execute through verified MIR. `shared? T` is an exact source shorthand. Optional truthiness, implicit unwrap, external optional signatures, optional references, and aliases to optional shared owners remain rejected. |
...
|---|---|---|
| [Compositional optional types](OPTIONAL_VALUES.md#frozen-compositional-extension) | **Partially implemented** | Recursive identities, owning lifecycle, one-layer checked access, aliases, and internal callable boundaries execute for arbitrarily nested optionals. Expected-type-directed `some(...)` constructs one explicit present layer, and chained `!` checks one layer at a time. Optional inline arrays `T[]?` execute across supported fields, statics, internal calls and dispatch, initializer overloads, array elements, and checked array-payload aliases. `shared T?` and `shared? T?` boxes remain outside this design. |
