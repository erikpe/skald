...
| [Remaining object-cast migration and shared extension](OBJECT_CASTS.md) | **Frozen design** | `(shared T) source`, eventual removal of scoped `narrow`, and future copy allocation through `new T((T) source)` remain planned. Plain casts and their implemented owning consumers never allocate. |
