...
| [Zero-default static fields](STATIC_FIELDS.md) | **Primitive, inline-optional, and optional-owner execution implemented** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and centralized zero-default validation. Class- and module-qualified primitive, inline-optional, and `shared? T` operations lower through receiver-free identity-based HIR and verified always-live MIR roots to deterministic private x86-64 zero slots. Optional owners preserve ordinary copy/adopt/move/release, secured unwrap, replacement, and process-lifetime final-owner behavior. Inline-array uses remain staged behind `TYP043`. |
