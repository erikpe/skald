...
|---|---|---|
| [Source and declarations](MODULES_AND_INTEROP.md#current-compilation-unit) | **Implemented contract** | One UTF-8 `.ska` source file; ASCII identifiers; line comments; top-level functions, exact-symbol external functions, nominal classes, and interfaces. |
...
| [Multiple files and modules](MODULES_AND_INTEROP.md#frozen-initial-module-system) | **Frozen design** | The settled whole-program system defines path-derived modules, anonymous composable roots, `::` qualification, explicit module and selective imports, private-by-default top-level declarations, rejected cycles, file or logical entry selection, controlled standard-library lookup, deterministic filesystem resolution and identities, and compatible external-ABI coalescing. Inactive compiler-layer APIs load deterministic reachable parsed graphs, collect per-module declaration indexes, build separate exact module and selective ordinary bindings, resolve imported public declarations once without re-exporting, and coalesce compatible foreign declarations behind verified compilation-wide link identities. Active driver integration is not implemented. The single-file adapter still rejects imports, and separate compilation and package distribution remain deferred. |
