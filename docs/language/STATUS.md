...
|---|---|---|
| [Complete explicit primitive cast matrix](TYPES_AND_VALUES.md#frozen-complete-explicit-primitive-cast-matrix) | **Implementation in progress** | All twenty-five explicit casts among `i64`, `u64`, `u8`, `f64`, and `bool` have settled contracts. Syntax, resolution, and typed HIR use one complete-matrix model; MIR represents and verifies all twenty-two non-failing cells as pure operations, and x86-64 executes all of them from direct verified MIR. Source admission remains limited to the nine integer cells; enabling every non-failing cell from source is next. |
