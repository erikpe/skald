...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design; verified MIR implemented** | The compiler accepts and types recursive invariant inline/shared arrays, construction, projection, replacement, slices, and aliases into deterministic HIR, then lowers them to verified target-independent MIR. MIR makes canonical types, storage roles, generated loops, ownership transfers, element lifecycle, checked positions/slices, failure edges, cleanup, and anchors explicit without target layout facts. The x86-64 backend rejects verified array MIR structurally, so arrays are not executable yet. |
