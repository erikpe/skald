...
|---|---|---|
| [Compositional optional types](OPTIONAL_VALUES.md#frozen-compositional-extension) | **Partially implemented** | Recursive identities and owning lifecycle now execute for arbitrarily nested optionals in locals, fields, statics, and array elements. Expected-type-directed `some(...)` constructs one explicit present layer. Nested checked access and callable integration remain for CO6, while optional inline arrays `T[]?` remain gated for CO7. `shared T?` and `shared? T?` boxes remain outside this design and require a later roadmap. |
