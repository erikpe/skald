...
| [Standard I/O](IO.md) | **Implemented contract** | Explicitly imported whole-stream reads and exact `Str` writes execute as ordinary Skald code over five private `u8[]` intrinsics. Type-named primitive `println_<type>` helpers compose canonical `Str` conversions with stdout and one ASCII line feed. Reads grow geometrically through EOF and close successful files; writes complete partial transfers. Runtime ABI version 7, dedicated HIR, verified MIR, checked x86-64 pointer/length lowering, exact host calls, and stable all-or-panic failures are implemented. Scalar observability output remains available unchanged. |
