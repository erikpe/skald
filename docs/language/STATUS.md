...
| [Primitive integer casts](TYPES_AND_VALUES.md#explicit-integer-casts) | **Implemented contract** | All nine explicit total two's-complement/modulo casts among `i64`, `u64`, and `u8` are accepted and verified through target-independent MIR. The x86-64 target returns a structured unsupported-feature error until INT4. |
