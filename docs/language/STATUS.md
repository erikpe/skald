...
| [Zero-default static fields](STATIC_FIELDS.md) | **Typed primitive frontend implemented** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and centralized zero-default validation. Class- and module-qualified primitive reads, writes, and `ref`/`mut ref` arguments lower to receiver-free identity-based HIR places and compose with ordinary primitive consumers. Accepted owning types remain declaration-only (`TYP043` on use), and `TYP044` stops static programs before MIR/native lowering. |
