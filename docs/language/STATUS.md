...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Copy allocation implemented** | Compatible same/ancestor/interface/`Obj` owner views cross internal calls without reallocating or replacing metadata. Shared-backed aliases, receivers, checked places, and `new T(copy source)` use verified source and anchor lifetimes. Shared fields execute as aligned owning graph edges, and strong cycles deliberately leak. Dynamic-type-preserving cloning is deferred. |
| [Shared object-cast extension](OBJECT_CASTS.md) | **Implemented contract** | `(shared T) source` preserves one allocation, dynamically checks when required, copies named owners, and transfers produced owners. It never allocates, slices, or copies payload. Distinct allocation is explicit through `new T(copy source)`. |
