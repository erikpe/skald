...
|---|---|---|
| [Explicit array element-list construction](ARRAYS.md#frozen-explicit-element-list-construction) | **Frozen design** | Typed `T[]{...}` and `new T[]{...}` determine length from an ordered list, allocate unpublished backing before element effects, initialize each exact stored element destination once from left to right, and publish only a complete prefix. Existing class, optional, nested-array, and shared-owner destination rules compose without default construction or assignment, and no runtime ABI extension is required. The current compiler rejects the brace forms; implementation-roadmap work may now be planned. |
