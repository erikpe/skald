...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Polymorphic owners and executable fields** | Compatible same/ancestor/interface/`Obj` owner views cross internal call boundaries without reallocating or replacing metadata. Stable shared locals and value parameters support inherited field access, mutable pointee methods, virtual/interface dispatch, and `is`. Shared fields execute as aligned owning graph edges, and strong cycles deliberately leak. Shared-backed aliases, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
