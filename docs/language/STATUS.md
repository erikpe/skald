...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Exact-target owners and executable fields** | Exact-class shared locals initialize and assign from named owners or ordinary allocations through explicit verified copy/adopt/release/move operations. Same-target owners cross all internal callable kinds. Shared fields execute on x86-64 as aligned one-word owning edges through initialization, secure replacement, user and synthesized copy lifecycle, inheritance, reverse destruction, and recursive dynamic finalization. Polymorphic views, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
