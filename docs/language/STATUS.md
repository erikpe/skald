...
|---|---|---|
| [Produced exact-class read-only alias arguments](ALIASES_AND_OWNERSHIP.md#frozen-produced-read-only-alias-arguments) | **Frozen design** | A construction, exact-class-returning internal call, canonical class literal, grouping, or supported checked cast materializes one hidden caller-owned object in verified MIR and binds it directly to compatible `ref` class/interface/`Obj` parameters. The owner survives later arguments and the complete call, retains exact complete-object provenance, and joins reverse full-expression cleanup without copying. Native execution and ABI evidence remain staged. `mut ref`, primitives, optional containers, arrays, raw shared handles, local aliases, and external alias signatures remain excluded. |
