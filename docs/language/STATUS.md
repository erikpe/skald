...
| [Primitive string conversions](STRINGS.md#frozen-primitive-textual-conversions) | **Frozen design** | Ten explicit `Str.from_<type>` and `Str.to_<type>` methods cover all five primitives. Canonical ASCII boolean and integer formatting is implemented in `std::str`; optional parsing and shortest round-tripping decimal `f64` formatting remain frozen but unavailable. |
...
| Broader standard library | **Open question** | The canonical string module and initial whole-stream I/O surface are implemented, and primitive textual conversions are frozen separately. Collections, general formatting and parsing, and broader library organization remain open. |
