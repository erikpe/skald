...
|---|---|---|
| [Strings](STRINGS.md) | **Partial (STR0)** | Raw-byte literals are lexed, decoded into syntax nodes, and add a conditional compiler-owned `std::str` dependency through ordinary providers. Provider-less use is diagnosed and no literal reaches typed HIR yet. Exact `Str` validation, intrinsic typed production, immortal backing, execution, and standard-library operations follow the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
