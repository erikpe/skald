...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Frozen design** | Non-null `shared T`, allocation only through concrete `new`, copy/adopt/release value semantics, non-atomic `u64` strong counts, dynamic complete-object destruction, hidden call/cast anchors, permitted strong-cycle leaks, and a minimal allocation ABI are settled but not implemented. |
| [C-style object casts](OBJECT_CASTS.md) | **Frozen design** | `(T) source` selects a checked non-owning place, `(shared T) source` preserves an existing allocation, inline destinations copy from class views, impossible casts are rejected, dynamic failure terminates, and `narrow` is planned for removal. |
