...
| [Declaring-class member privacy](CLASSES_AND_LIFECYCLE.md#declaring-class-privacy) | **Implemented contract** | Public-by-default fields and instance methods; contextual `private`; exact declaring-class access across all class-owned bodies and receiver forms; inherited namespace preservation; and privacy erasure before HIR. Private methods are direct and do not satisfy interfaces. |
...
|---|---|---|
| [Strings](STRINGS.md) | **Partial (STR3)** | Raw-byte literals are lexed and decoded, conditionally reach the exact provider-owned `std::str::Str`, validate its private descriptor contract, and become deterministic intrinsic produced values through native x86-64 execution. Verified MIR distinguishes immutable immortal backing; the backend pools exact bytes, emits shared-array metadata relocations, materializes descriptors through class layout, and applies ordinary lifecycle with sentinel-aware retain/release. Provider-less use and invalid language items are diagnosed before HIR. Standard-library string operations follow the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
...
| [Primitive integer comparisons and casts](TYPES_AND_VALUES.md#frozen-primitive-integer-comparisons-and-casts) | **Frozen design** | Exact-type `==`, `!=`, `<`, `<=`, `>`, and `>=` for `i64`, `u64`, and `u8` are accepted through verified target-independent MIR; the x86-64 target rejects them until backend realization. All nine explicit total two's-complement/modulo casts are settled but primitive-keyword cast targets remain unaccepted. |
