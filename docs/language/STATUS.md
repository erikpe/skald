...
| [Primitive string conversions](STRINGS.md#frozen-primitive-textual-conversions) | **Frozen design** | Ten explicit `Str.from_<type>` and `Str.to_<type>` methods cover all five primitives. Formatting is canonical ASCII and locale-independent; parsing consumes the complete string and returns `T?`, with shortest round-tripping decimal output for `f64`. The methods are not yet installed in `std::str`. |
