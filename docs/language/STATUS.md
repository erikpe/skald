...
|---|---|---|
| [Standard I/O](IO.md) | **Compiler/runtime foundation** | Four planned explicitly imported whole-stream `std::io` functions over `Str`. Runtime ABI version 7 and the compiler implement five private `u8[]` intrinsic identities end to end through dedicated HIR, verified MIR, checked x86-64 pointer/length lowering, and exact host calls. Public function bodies remain planned; current scalar output remains bootstrap interoperation. |
