...
|---|---|---|
| [Explicit array element-list construction](ARRAYS.md#frozen-explicit-element-list-construction) | **Frozen design; typed HIR retained** | Typed `T[]{...}` and `new T[]{...}` determine length from an ordered list, allocate unpublished backing before element effects, initialize each exact stored element destination once from left to right, and publish only a complete prefix. Syntax and resolution retain exact source structure; type checking selects exact primitive, class, optional, nested-array, shared-owner, and optional-owner destination plans in HIR. A structured executable-lowering gate rejects the mode until verified MIR lands. No default construction or assignment is introduced, and no runtime ABI extension is required. |
