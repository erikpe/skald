...
|---|---|---|
| [Strings](STRINGS.md) | **Partial (STR1)** | Raw-byte literals are lexed and decoded, conditionally reach the exact provider-owned `std::str::Str`, validate its private descriptor contract, and become deterministic intrinsic produced values in resolved IR and typed HIR. Provider-less use and invalid language items are diagnosed before HIR; the complete driver stops before MIR with a structured not-yet-executable diagnostic. Immortal backing, execution, and standard-library operations follow the [implementation roadmap](../roadmaps/STRINGS_ROADMAP.md). |
