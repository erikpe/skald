...
| Area | Maturity | Current compiler boundary |
|---|---|---|
| [Primitive operator profile](TYPES_AND_VALUES.md#frozen-primitive-operator-profile) | **Frozen design** | Exact primitive unary and binary matrices; no implicit conversion; frozen precedence with non-associative comparison/`is`; wrapping integer arithmetic; floor signed division and divisor-sign remainder; checked `u64` shift counts; IEEE binary64 division and unordered comparison; exact boolean equality and mandatory short-circuit `&&`/`||`; selected-path full-expression cleanup; and three compiler-known panic reasons. The compiler currently supports only the subsets listed under implemented primitive expressions, integer bitwise operators, integer comparisons, eager boolean operators, and short-circuit boolean expressions. The remaining frozen tokens, operations, failure reasons, and target lowering are not accepted or emitted. |
