...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design; partial native execution** | The compiler accepts and lowers the complete frozen array profile to verified target-independent MIR. On x86-64, non-shared inline arrays containing primitives, optionals, exact classes, and recursively nested inline arrays execute empty or dynamic default construction, immutable `len()`, checked positive and negative-relative element access, named deep copy, produced-backing adoption, arbitrary-length replacement, class fields, internal value parameters/results, checked allocation, conditional optional lifecycle, and deterministic reverse cleanup. Shared outer arrays, shared-owner elements, slices, and array aliases remain structured backend errors. |
