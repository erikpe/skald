...
| [Multiple files and modules](MODULES_AND_INTEROP.md#frozen-initial-module-system) | **Frozen design** | The settled whole-program system defines path-derived modules, anonymous composable roots, `::` qualification, explicit module and selective imports, private-by-default top-level declarations, rejected cycles, file or logical entry selection, controlled standard-library lookup, deterministic filesystem resolution and identities, and compatible external-ABI coalescing. Inactive compiler-layer APIs now load deterministic reachable parsed graphs and collect them into per-module declaration indexes plus one flat semantic program; local unqualified lookup remains module-owned. Import bindings, qualified and selective lookup, external-ABI coalescing, and active driver integration are not implemented. The single-file adapter still rejects imports, and separate compilation and package distribution remain deferred. |
