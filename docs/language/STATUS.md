...
| [Static fields](STATIC_FIELDS.md) | **Eager startup implemented; reverse shutdown planned** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and separate zero-default versus explicit stored-value validation. Explicit expressions use ordinary construction, copy, owner-transfer, optional, string, and array semantics. Whole-program effects produce evidenced dependencies, deterministic activation and reverse-shutdown order, and source diagnostics for self-dependencies and cycles. Final-MIR verification checks publication, cleanup, ownership, destination non-escape, transitions, effects, edges, and plan indices. The x86-64 backend emits private aligned slots, ordinary initializer bodies, and one dependency-ordered program initializer called after the runtime marker and before entry without per-access lifecycle guards. Normal-return static cleanup remains pending. |
| Broader class and static state | **Frozen design and active roadmap** | Reverse normal-return shutdown is staged after implemented eager startup. Top-level globals, abstract/final forms, method/function overloads, reflection, and user-defined conversions remain outside the current contract. Declaring-class privacy, static methods, and ordinary initializer overloading are implemented separately above. |
