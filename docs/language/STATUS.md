...
| [C-style object casts](OBJECT_CASTS.md) | **Frozen design** | `(T) source` selects a checked non-owning place, `(shared T) source` preserves an existing allocation, inline and future copy-allocation destinations copy from class views, casts themselves never allocate, impossible casts are rejected, dynamic failure terminates, and `narrow` is planned for removal. |
