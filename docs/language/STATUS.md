...
| [Exact nominal classes](CLASSES_AND_LIFECYCLE.md#exact-nominal-classes) | **Implemented contract** | Inline class values; primitive and class fields; exactly one ordinary initializer; and statically selected read-only and mutable receiver methods. Dynamic dispatch remains unavailable. |
...
|---|---|---|
| [Dynamic polymorphism](POLYMORPHISM.md) | **Partial implementation** | Static hierarchy, lifecycle, inherited access, ancestor/`Obj` views, slicing, and x86-64 execution are implemented. HIR selects virtual-family calls; MIR represents and verifies virtual targets and complete-object receiver origins. Backend virtual/interface dispatch, type tests, and narrowing remain unavailable. |
