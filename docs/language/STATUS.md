...
| [Primitive string conversions](STRINGS.md#frozen-primitive-textual-conversions) | **Frozen design** | Ten explicit `Str.from_<type>` and `Str.to_<type>` methods cover all five primitives. Canonical ASCII boolean and integer formatting and optional parsing plus correctly rounded binary64 parsing are implemented in `std::str`; shortest round-tripping binary64 formatting remains frozen but unavailable. |
