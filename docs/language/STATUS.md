...
| [Bindings and scopes](FUNCTIONS_AND_CONTROL_FLOW.md#lexical-scopes-and-locals) | **Implemented contract** | Typed `var` locals, value parameters, lexical blocks, declaration-before-use for locals, nested shadowing, and duplicate rejection within one scope. Primitive local reassignment is tracked separately below. |
...
|---|---|---|
| [Primitive local reassignment](FUNCTIONS_AND_CONTROL_FLOW.md#frozen-primitive-local-reassignment) | **Frozen design** | An initialized `i64`, `u64`, `u8`, `f64`, or `bool` `var` local may be replaced by an exactly typed, once-evaluated source. Grouping preserves the selected lexical local identity; the store precedes source full-expression cleanup and changes no lifetime state. The parser accepts the source shape, but semantic classification and execution are not implemented. Primitive value-parameter rebinding and broader assignment forms remain excluded. |
