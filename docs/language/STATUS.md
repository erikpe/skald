...
|---|---|---|
| [Standard I/O](IO.md) | **Partially implemented contract** | Explicitly imported `write_stdout` and `write_stderr` execute as partial-write-safe Skald loops over `Str.to_bytes()` and the private `u8[]` intrinsic pipeline. Runtime ABI version 7, dedicated HIR, verified MIR, checked x86-64 pointer/length lowering, and exact host calls are implemented. `read_stdin` and `read_file` remain planned; scalar observability output remains available unchanged. |
