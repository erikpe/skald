...
|---|---|---|
| [Explicit copy construction](CLASSES_AND_LIFECYCLE.md#copy-construction-and-object-sources) | **Frozen design** | Ordinary `init` overload selection is implemented for direct construction and `super(arguments)`, and mode-only overload pairs are rejected. Target-directed `T(copy source)` remains constructor-roadmap work before shared ownership. |
