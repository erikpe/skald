...
| [Primitive string conversions](STRINGS.md#frozen-primitive-textual-conversions) | **Frozen design** | Ten explicit `Str.from_<type>` and `Str.to_<type>` methods cover all five primitives. Canonical ASCII boolean and integer formatting and optional parsing are implemented in `std::str`; correctly rounded binary64 parsing and shortest round-tripping binary64 formatting remain frozen but unavailable. |
