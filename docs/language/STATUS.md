...
|---|---|---|
| [Compositional optional types](OPTIONAL_VALUES.md#frozen-compositional-extension) | **Partially implemented** | Recursive source syntax, general grouping, left-to-right `?`/`[]` suffixes, canonical executable `(shared T)?`, and exact `shared? T` shorthand are implemented. Recursive interned identities, arbitrary executable `T??`, expected-type-directed `some(...)`, and optional inline arrays `T[]?` retain frozen contracts but currently stop at semantic gates. `shared T?` and `shared? T?` boxes remain outside this design and require a later roadmap. |
