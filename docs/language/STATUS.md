...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Exact local value semantics** | Exact-class shared locals initialize and assign from named owners or ordinary allocations through explicit verified copy/adopt/release/move operations. The x86-64 backend executes checked allocation and retain, secure-before-release assignment, full-expression ownership boundaries, normal cleanup, dynamic exact-class destruction, and last-owner deallocation. Calls/results, fields, polymorphic views, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
