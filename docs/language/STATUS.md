...
| [Exact nominal classes](CLASSES_AND_LIFECYCLE.md#exact-nominal-classes) | **Implemented contract** | Inline class values; primitive and exact-class fields; exactly one ordinary initializer; and statically selected read-only and mutable receiver methods. Dynamic dispatch remains unavailable. |
| [Static inheritance semantics](POLYMORPHISM.md#hierarchies-and-declaration-namespaces) | **Frontend/HIR contract** | Canonical single-base identity; mandatory `super(...)`; complete lifecycle composition; inherited fields and direct methods through identity-based base projections; access-preserving class/`Obj` alias views; and explicit owning slices. MIR representation, layout, and execution remain unavailable. |
...
|---|---|---|
| [Inheritance and polymorphism](POLYMORPHISM.md) | **Frozen design** | Static hierarchy, lifecycle, inherited access, ancestor/`Obj` views, and slicing reach typed HIR and stop before MIR. MIR/base layout, execution, virtual and interface dispatch, type tests, and narrowing remain unavailable. |
