...
| [Intrinsic declarations](MODULES_AND_INTEROP.md#intrinsic-function-declarations) | **Implemented contract** | Contextual bodyless top-level `intrinsic fn`; only the exact public `std::error::panic(message: std::str::Str) -> unit` declaration is accepted. Imports and calls resolve by stable identity, but a call is rejected before HIR until executable panic lowering is implemented. |
...
|---|---|---|
| [Panic and compiler-known unrecoverable failures](ERRORS.md#frozen-panic-design) | **Frozen design** | Canonical imported `std::error::panic(Str)` is a non-returning call statement; explicit messages and the closed catalog of compiler-known failures use one length-delimited reporter. Panic is uncatchable and non-unwinding, while compiler/runtime defects remain hard traps. The version-6 reporter is implemented and directly tested, but no source panic is accepted and no compiler-emitted path calls it yet. |
