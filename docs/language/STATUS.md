...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Exact-target owners; target-independent fields** | Exact-class shared locals initialize and assign from named owners or ordinary allocations through explicit verified copy/adopt/release/move operations. Same-target owners cross all internal callable kinds. Shared fields now participate in exact initialization, secure replacement, user and synthesized copy lifecycle, inheritance, and reverse destruction through verified MIR. The x86-64 backend executes field-free owner lifetimes and deliberately rejects shared-field layout until the next slice. Polymorphic views, anchors, explicit copy allocation, and shared-owner casts remain gated. Dynamic-type-preserving cloning is deferred. |
