...
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Implemented contract** | Non-null strong owner handles, ordinary and exact-class copy allocation, named copy and produced transfer, secure assignment, internal parameters/results, owning fields, compatible polymorphic views, deterministic dynamic last-owner destruction, and verified hidden borrow anchors execute on x86-64. `*owner` is required for general pointee-place consumers, `owner->member` crosses one shared edge, and `.` never implicitly dereferences a handle. Strong cycles deliberately leak; weak ownership, whole-pointee assignment, and dynamic cloning remain excluded. The frozen `shared? T` optional-owner design is not yet implemented. |
...
|---|---|---|
| [Optional values](OPTIONAL_VALUES.md) | **Frozen design; inline optional slice implemented** | Primitive and exact-class `T?` owning locals, fields, internal parameters/results, methods, interfaces, overrides, initializer overloads, conditional class lifecycle, and bounded checked class payload views execute through verified MIR and one hidden-destination aggregate ABI. Optional truthiness, implicit unwrap, and external optional signatures are rejected. Optional shared owners and optional-container aliases remain at `TYP035`. |
