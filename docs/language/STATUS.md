...
|---|---|---|
| [Complete explicit primitive cast matrix](TYPES_AND_VALUES.md#frozen-complete-explicit-primitive-cast-matrix) | **Implementation in progress** | All twenty-five explicit casts among `i64`, `u64`, `u8`, `f64`, and `bool` have settled contracts. Syntax, resolution, and typed HIR use one complete-matrix model; MIR represents and verifies all twenty-two non-failing cells as pure operations. Source admission remains limited to the nine integer cells. Direct verified MIR additionally executes `f64`/`bool` identity and every boolean-boundary cast; integer-to-`f64` target execution is next. |
