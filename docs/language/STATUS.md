...
| [Static fields](STATIC_FIELDS.md) | **Zero-default execution complete; initializer front end staged** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and centralized zero-default validation. Class- and module-qualified primitive, inline-optional, `shared? T`, and inline-array operations lower through receiver-free identity-based HIR and verified always-live MIR roots to deterministic private x86-64 zero slots. Optional declaration expressions are retained with exact spans and stable field-derived identities, then resolved after whole-program declarations in a receiver-free declaring-class context. They currently report `TYP043` before HIR and do not execute. |
| Broader class and static state | **Frozen design and active roadmap** | Typed static declaration initialization, dependency and cycle analysis, eager startup, reverse normal-return shutdown, and exact-class or non-optional-owner static values are staged by the active roadmap. Top-level globals, abstract/final forms, method/function overloads, reflection, and user-defined conversions remain outside the current contract. Declaring-class privacy, static methods, and ordinary initializer overloading are implemented separately above. |
