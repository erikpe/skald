...
|---|---|---|
| [Optional values](OPTIONAL_VALUES.md) | **Frozen design; owning values implemented** | Primitive and exact-class `T?` plus class/interface/`Obj` `shared? T` locals, fields, internal parameters/results, methods, interfaces, overrides, initializer overloads, conditional lifecycle, checked class payload views, secured shared unwrap, one-word optional-owner ABI, casts after unwrap, and anchors execute through verified MIR. Optional truthiness, implicit unwrap, external optional signatures, and optional-container aliases remain rejected. |
