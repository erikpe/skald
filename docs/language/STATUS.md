...
| [Zero-default static fields](STATIC_FIELDS.md) | **Primitive execution implemented** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and centralized zero-default validation. Class- and module-qualified primitive reads, writes, and `ref`/`mut ref` arguments lower through receiver-free identity-based HIR and verified always-live MIR roots to deterministic private x86-64 zero slots. Accepted owning types remain declaration-only (`TYP043` on use). |
