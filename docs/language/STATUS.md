...
| [Panic runtime traces](ERRORS.md#frozen-panic-runtime-traces) | **Implementation in progress** | Runtime ABI version 9 defines the fixed records and hidden TLS top and renders directly supplied newest-first chains allocation-free with a 256-row cap. The compiler still publishes no frames or metadata, so generated programs retain single-line panic output; backend maintenance, source attribution, default-on emission, and `--omit-runtime-trace` remain pending. The source grammar and panic API are unchanged. |
