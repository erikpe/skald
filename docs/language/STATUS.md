...
|---|---|---|
| [Explicit array element-list construction](ARRAYS.md#frozen-explicit-element-list-construction) | **Frozen design; all legal element families execute; hardening pending** | Typed `T[]{...}` and `new T[]{...}` determine length from an ordered list, allocate unpublished backing before element effects, initialize each exact stored element destination once from left to right, and publish only a complete prefix. Primitive slots store exact values. Exact-class slots use final-destination initialization, call-result placement, or selected copy construction. Inline optionals reuse ordinary absence, payload lifecycle, presence publication, and conditional cleanup. Nested inline arrays deep-copy named sources or adopt produced backings with exact identities. Shared-owner slots retain named compatible owners or adopt produced allocations, calls, casts, and shared arrays; optional owner slots reuse zero-niche conditional ownership. Inline and shared outer arrays use the same verified protocol, cleanup is reverse and category-correct, no default construction or assignment is introduced, and runtime ABI version 8 is unchanged. |
