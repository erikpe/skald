...
| [Primitive integer comparisons and casts](TYPES_AND_VALUES.md#frozen-primitive-integer-comparisons-and-casts) | **Frozen design** | Exact-type `==`, `!=`, `<`, `<=`, `>`, and `>=` for `i64`, `u64`, and `u8`, plus all nine explicit total two's-complement/modulo casts among those types, are settled but not implemented. The compiler currently accepts neither comparison operators nor primitive-keyword cast targets. |
