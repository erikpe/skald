...
| [Call-scoped aliases](ALIASES_AND_OWNERSHIP.md#implemented-eligible-argument-places) | **Implemented contract** | `ref` and `mut ref` parameters over existing inline and explicitly dereferenced shared object places, including ancestor/interface/`Obj` views, forwarding, and deliberate overlap. Dereferenced stable shared owners borrow directly; fields, nested places, and produced shared owners use verified hidden call anchors. Direct produced inline exact-class sources are accepted through type checking and HIR only for read-only `ref`; their verified lifetime lowering remains staged. Aliases remain non-owning and access-restricted; local aliases and external alias signatures are unsupported. |
...
|---|---|---|
| [Produced exact-class read-only alias arguments](ALIASES_AND_OWNERSHIP.md#frozen-produced-read-only-alias-arguments) | **Frozen design** | A construction, exact-class-returning internal call, canonical class literal, grouping, or supported checked cast may materialize one hidden caller-owned object and bind it directly to compatible `ref` class/interface/`Obj` parameters. Type checking, initializer applicability, diagnostics, and produced-view HIR are implemented. Proving that the owner survives later arguments and the complete call, then joins reverse full-expression cleanup without copying, remains staged MIR/backend work. `mut ref`, primitives, optional containers, arrays, raw shared handles, local aliases, and external alias signatures remain excluded. |
