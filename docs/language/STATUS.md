...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Typed HIR vocabulary** | Shared locals, value parameters, results, and fields have canonical class/interface/`Obj` targets. HIR records named versus produced owner provenance, copy/adopt transfers, and ordinary `new C(arguments)` with one selected initializer. MIR lowering deliberately rejects shared HIR; allocation execution, retain/release, dynamic destruction, anchors, explicit copy allocation, shared-owner casts, and the minimal allocation ABI remain unimplemented. Dynamic-type-preserving cloning is deferred. |
