...
| [Panic runtime traces](ERRORS.md#frozen-panic-runtime-traces) | **Implementation in progress** | Runtime ABI version 9 renders newest-first chains allocation-free with a 256-row cap. Enabled x86-64 backend emission maintains one linked 16-byte frame per source activation, records source/external calls, generated-helper/runtime operations, and reporter failure edges with inline local-exec TLS, and emits deterministic requested-only metadata. Default-on emission and the public `--omit-runtime-trace` option remain pending; the production driver deliberately omits tracing, so ordinary generated programs retain single-line panic output. The source grammar and panic API are unchanged. |
...
| Broader standard library | **Open question** | The canonical string module, primitive textual conversions, initial whole-stream I/O, and process arguments are implemented. Collections, general formatting and parsing, and broader library organization remain open. |
