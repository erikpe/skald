...
| [Standard I/O](IO.md) | **Implemented contract** | Explicitly imported whole-stream reads and exact `Str` writes execute as ordinary Skald code over five private `u8[]` intrinsics. Type-named primitive `println_<type>` helpers compose canonical `Str` conversions with stdout and one ASCII line feed. Reads grow geometrically through EOF and close successful files; writes complete partial transfers. Runtime ABI version 8, dedicated HIR, verified MIR, checked x86-64 pointer/length lowering, exact host calls, and stable all-or-panic failures are implemented. Primitive text conversion remains entirely in the standard library. |
| [Process arguments](PROCESS.md) | **Implemented contract** | Explicitly imported `std::process::args() -> Str[]` returns a fresh owning snapshot of the complete raw-byte host invocation vector, including the invocation name at index zero. The Linux library implementation reads `/proc/self/cmdline` through existing whole-file I/O; `main`, grammar, compiler phases, and runtime ABI version 8 remain unchanged. |
...
|---|---|---|
| [Explicit array element-list construction](ARRAYS.md#frozen-explicit-element-list-construction) | **Frozen design; primitive, exact-class, inline-optional, and recursively nested inline-array execution implemented** | Typed `T[]{...}` and `new T[]{...}` determine length from an ordered list, allocate unpublished backing before element effects, initialize each exact stored element destination once from left to right, and publish only a complete prefix. Inline and shared-outer primitive lists execute through verified MIR and x86-64. Exact-class lists additionally use final-slot initializer and call-result placement or selected copy construction, preserve grouping and full-expression temporaries, and destroy completed elements in reverse order. Supported inline optionals reuse ordinary absence, injection, conditional payload copying, direct payload placement, presence publication, and conditional reverse cleanup before advancing the outer prefix. Nested inline arrays recursively deep-copy named sources, adopt produced backings exactly once, preserve jagged lengths and exact identities, and clean up in recursive reverse order. Shared-owner and optional-owner plans remain behind the structured executable-lowering gate. No default construction or assignment is introduced, and runtime ABI version 8 is unchanged. |
...
| [Zero-default static fields](STATIC_FIELDS.md) | **Complete initial profile implemented** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and centralized zero-default validation. Class- and module-qualified primitive, inline-optional, `shared? T`, and inline-array operations lower through receiver-free identity-based HIR and verified always-live MIR roots to deterministic private x86-64 zero slots. Optional owners and replaceable arrays preserve ordinary ownership, backing anchors, aliases, slices, byte-I/O buffers, replacement cleanup, and deliberate process-lifetime retention of final contents. |
| Broader class and static state | **Open question** | Static declaration initializers and destruction, exact-class or non-optional-owner static initialization, top-level globals, abstract/final forms, method/function overloads, reflection, and user-defined conversions are not current language contracts. Declaring-class privacy, static methods, and ordinary initializer overloading are implemented separately above. |
