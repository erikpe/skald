...
| [Object vectors](VECTORS.md) | **Implemented contract** | Explicitly imported `std::vec::VecObj` stores non-null `shared Obj` elements in independent inline vector storage, with default and requested capacity, geometric growth, checked positive and negative indexing, push, pop, last, replacement, clear, and prompt removed-owner release. It is ordinary standard-library source and adds no generic, indexing-protocol, compiler, or runtime surface. |
...
|---|---|---|
| [Shared optional boxes](OPTIONAL_VALUES.md#shared-optional-boxes) | **Frozen design; exact local access implemented** | `shared P?` is a non-null owner of an immutable published `P?` wrapper; `(shared P?)?` is its canonical optional owner and `shared? P?` is exact shorthand. Exact local construction, ownership, recursive finalization, presence, wrapper copy, read-only aliasing, one-layer unwrap, and contained-value use execute through verified MIR and x86-64 code. Polymorphic object-box access and broader stored/callable owner positions remain staged. |
...
| Broader class and global state | **Open question** | Top-level or module-owned globals, abstract/final forms, method/function overloads, reflection, and user-defined conversions remain outside the current contract. Implemented declaring-class privacy, static methods, ordinary initializer overloading, and class-owned static fields are documented separately above. |
| Broader standard library | **Open question** | The canonical string module, primitive textual conversions, initial whole-stream I/O, process arguments, and initial shared-object `VecObj` are implemented. Primitive and exact-inline vector families, broader collections, general formatting and parsing, and broader library organization remain open. |
