...
|---|---|---|
| [Explicit copy construction and overloaded direct-base initialization](CLASSES_AND_LIFECYCLE.md#ordinary-initializer-overloads) | **Partially implemented** | Direct `T(arguments)` construction selects among one or more ordinary `init` overloads by static applicability and unique most-specific parameter types; mode-only overload pairs are rejected. Applying that engine to `super(arguments)` and implementing target-directed `T(copy source)` remain constructor-roadmap work before shared ownership. |
