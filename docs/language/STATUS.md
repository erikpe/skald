...
|---|---|---|
| [Standard I/O](IO.md) | **Compiler/runtime foundation** | Four planned explicitly imported whole-stream `std::io` functions over `Str`. Runtime ABI version 7 implements the host byte operations, and the installed module plus compiler implement five private `u8[]` intrinsic identities, dedicated HIR, and verified target-independent MIR with checked ranges and backing anchors. x86-64 lowering and public function bodies remain planned; current scalar output remains bootstrap interoperation. |
