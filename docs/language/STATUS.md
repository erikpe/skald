...
|---|---|---|
| [Complete explicit primitive cast matrix](TYPES_AND_VALUES.md#frozen-complete-explicit-primitive-cast-matrix) | **Implementation in progress** | All twenty-five explicit casts among `i64`, `u64`, `u8`, `f64`, and `bool` have settled contracts. All twenty-two non-failing cells are accepted from source, represented and verified as pure MIR, and executed inline on x86-64. The three checked `f64`-to-integer cells have explicit verified MIR diamonds and complete inline x86-64 execution, but remain temporarily source-gated. |
