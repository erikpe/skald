...
| [`while`, `break`, and `continue`](FUNCTIONS_AND_CONTROL_FLOW.md#while-loops-and-loop-exits) | **Frozen design** | Mandatory-block statement `while` has an exact-`bool` condition, per-test full-expression cleanup, a fresh body scope per iteration, deterministic loop-exit cleanup, nearest-loop value-free `break` and `continue`, and conservative fallthrough for definite return. None are accepted yet; implementation is ordered as `while` followed by the two exits. |
