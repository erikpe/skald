...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design; partial native execution** | The compiler accepts and lowers the complete frozen array profile to verified target-independent MIR. On x86-64, primitive inline local `i64`, `u64`, `u8`, `f64`, and `bool` arrays execute empty or dynamic default construction, immutable `len()`, exact zero/false initialization, checked positive and negative-relative element reads and mutation, checked allocation, and normal cleanup. Non-local ownership, copying/replacement, nontrivial elements, shared outer arrays, slices, and detachable aliases remain structured backend errors. |
