...
|---|---|---|
| [Shared optional boxes](OPTIONAL_VALUES.md#shared-optional-boxes) | **Frozen design** | `shared P?` is a non-null owner of an immutable published `P?` wrapper; `(shared P?)?` is its canonical optional owner and `shared? P?` is exact shorthand. Exact non-object targets and polymorphic class/base/interface/`Obj` object-box views use explicit dereference, existing optional guards, one-word owners, exact metadata/finalization, and runtime ABI version 9. Construction, target identities, verified ownership, native realization, and stored-position integration remain to be implemented by the active roadmap. |
