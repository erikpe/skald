...
|---|---|---|
| [Process arguments](PROCESS.md) | **Frozen design** | A future explicitly imported `std::process::args() -> Str[]` returns a fresh owning snapshot of the complete raw-byte host invocation vector, including the invocation name at index zero. The initial Linux library implementation reads `/proc/self/cmdline` through existing whole-file I/O; `main`, grammar, compiler phases, and runtime ABI version 8 remain unchanged. |
...
| Broader standard library | **Open question** | The canonical string module, primitive textual conversions, and initial whole-stream I/O surface are implemented; process arguments have a separate frozen contract. Collections, general formatting and parsing, and broader library organization remain open. |
