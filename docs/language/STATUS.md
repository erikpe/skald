...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design; partial compiler implementation** | The compiler lexes, parses, and resolves recursive invariant array identities, then types internal owning declarations and empty/default-length/explicit-copy construction into deterministic HIR. HIR records exact element default/copy/assignment/destruction plans, inline versus shared allocation, and named deep-copy versus produced-backing adoption; recursive class/array capability analysis terminates without treating backing as inline containment. Invalid elements, unavailable defaults/copies, length types and static maximums, ownership mismatches, interface contracts, and external signatures are diagnosed. Projection, replacement, slices, and aliases still stop at `TYP035`; all valid array HIR stops at the deliberate HIR-to-MIR `TYP035` gate, so arrays are not executable. |
