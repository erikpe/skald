...
|---|---|---|
| [Complete explicit primitive cast matrix](TYPES_AND_VALUES.md#frozen-complete-explicit-primitive-cast-matrix) | **Implementation in progress** | All twenty-five explicit casts among `i64`, `u64`, `u8`, `f64`, and `bool` have settled contracts. Syntax and resolution preserve all five primitive targets, and typed HIR has one complete-matrix semantic model; type checking, MIR, and native execution remain limited to the nine integer cells. Verified pure primitive-cast MIR is the next implementation boundary. |
