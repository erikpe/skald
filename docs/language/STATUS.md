...
|---|---|---|
| [Dynamic polymorphism](POLYMORPHISM.md) | **Partial implementation** | Static hierarchy, lifecycle, inherited access, ancestor/interface/`Obj` views, slicing, opt-in virtual methods, exact inherited interface conformance, forwarding, and interface calls execute on x86-64. Type tests and scoped checked narrowing lower through verified target-independent MIR; x86-64 execution remains pending. |
