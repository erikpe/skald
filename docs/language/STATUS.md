...
| [Panic runtime traces](ERRORS.md#frozen-panic-runtime-traces) | **Implementation in progress** | Runtime ABI version 9 renders newest-first chains allocation-free with a 256-row cap. Enabled x86-64 backend emission now maintains one linked 16-byte frame per source activation using inline local-exec TLS, alongside deterministic requested-only semantic metadata. Precise interior location attribution, default-on emission, and `--omit-runtime-trace` remain pending; the production driver deliberately omits tracing, so ordinary generated programs retain single-line panic output. The source grammar and panic API are unchanged. |
