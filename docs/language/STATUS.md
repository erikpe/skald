...
|---|---|---|
| [Arrays](ARRAYS.md) | **Frozen design** | The compiler lexes, parses, and resolves recursive array types to deterministic canonical identities, including inline/shared ownership grouping, nested arrays, construction modes, projections, and assignment destinations. Array semantics deliberately stop at the structured type-checking boundary `TYP035`; no array form reaches HIR or execution yet. Deep named copies, produced-backing adoption, default construction, jagged nesting, deterministic element lifetime, and backing anchors remain frozen for later implementation; rich initialization, inline optional array payloads, slice views, resizing, iteration, and external ABI remain deferred. |
