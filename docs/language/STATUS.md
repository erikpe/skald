...
|---|---|---|
| [Compositional optional types](OPTIONAL_VALUES.md#frozen-compositional-extension) | **Frozen design** | `(shared T)?` is canonical with implemented `shared? T` retained as exact shorthand; recursive interned optional identities, arbitrary `T??` nesting, expected-type-directed `some(...)`, one-layer injection and unwrap, and optional inline arrays `T[]?` have settled language and compiler contracts. The current grammar and compiler still reject the new forms. `shared T?` and `shared? T?` boxes remain outside this design and require a later roadmap. |
