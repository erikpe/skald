...
|---|---|---|
| [Produced exact-class read-only alias arguments](ALIASES_AND_OWNERSHIP.md#frozen-produced-read-only-alias-arguments) | **Frozen design** | A construction, exact-class-returning internal call, canonical class literal, grouping, or supported checked cast may materialize one hidden caller-owned object and bind it directly to compatible `ref` class/interface/`Obj` parameters. The owner survives later arguments and the complete call, then joins reverse full-expression cleanup without copying or changing the internal alias ABI. `mut ref`, primitives, optional containers, arrays, raw shared handles, local aliases, and external alias signatures remain excluded. The current compiler still diagnoses these direct produced inline arguments as non-place alias sources. |
