...
|---|---|---|
| [Standard I/O](IO.md) | **Frozen design** | Four explicitly imported whole-stream `std::io` functions over `Str`, implemented through private `u8[]` intrinsics and a narrow handle runtime boundary. Runtime ABI version 7 implements the host byte operations; the module and compiler intrinsics remain planned, and current scalar output remains bootstrap interoperation. |
