...
|---|---|---|
| [Shared ownership and heap allocation](SHARED_OWNERSHIP.md) | **Checked-place anchors implemented** | Compatible same/ancestor/interface/`Obj` owner views cross internal calls without reallocating or replacing metadata. Shared-backed alias arguments, receivers, and plain checked places borrow stable owners directly and anchor replaceable or produced owners through complete consumer cleanup. Shared fields execute as aligned owning graph edges, and strong cycles deliberately leak. Explicit copy allocation remains gated. Dynamic-type-preserving cloning is deferred. |
