...
| [Static fields](STATIC_FIELDS.md) | **Zero-default execution complete; explicit final lifecycle MIR verified** | Dedicated declarations have independent dense identity, ordinary-member collision and inheritance rules, declaring-class privacy, and separate zero-default versus explicit stored-value validation. Class- and module-qualified primitive, inline-optional, `shared? T`, and inline-array operations lower through receiver-free identity-based HIR and verified always-live MIR roots to deterministic private x86-64 zero slots. Explicit expressions resolve and type-check once under stable field-derived identities through ordinary construction, copy, owner-transfer, optional, string, and array semantics, then lower to independently identified preliminary MIR bodies. Whole-program effects produce evidenced static dependencies, deterministic activation and reverse-shutdown order, and source diagnostics for self-dependencies and cycles. Synthesis moves unchanged bodies into final activation/publication regions and adds exact-reverse typed destruction regions; final-MIR verification rechecks control flow, ownership, destination non-escape, transitions, effects, edges, and plan indices. `DRV001` currently stops valid initialized fields before backend startup execution. |
| Broader class and static state | **Frozen design and active roadmap** | Eager startup, reverse normal-return shutdown, and executable exact-class or non-optional-owner static values are staged after the implemented coordinator synthesis. Top-level globals, abstract/final forms, method/function overloads, reflection, and user-defined conversions remain outside the current contract. Declaring-class privacy, static methods, and ordinary initializer overloading are implemented separately above. |
