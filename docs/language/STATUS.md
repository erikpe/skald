...
| [Checked object-place casts](OBJECT_CASTS.md) | **Implemented contract** | Unary `(T) source` casts preserve access and complete-object provenance for immediate class/interface method receivers, alias arguments, primitive field reads, and supported field mutation. Static views project directly; dynamic failure terminates; produced inline sources remain alive in full-expression temporaries. Owning cast consumers are tracked separately below. |
...
| [Remaining object-cast integration](OBJECT_CASTS.md) | **Frozen design** | Owning inline initialization, assignment, arguments, and returns from a checked class place; `(shared T) source`; and eventual removal of scoped `narrow` remain planned. Casts themselves never allocate, and copy allocation remains explicit through future `new T((T) source)`. |
