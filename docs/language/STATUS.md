...
|---|---|---|
| [Shared optional boxes](OPTIONAL_VALUES.md#shared-optional-boxes) | **Frozen design; frontend staged** | `shared P?` is a non-null owner of an immutable published `P?` wrapper; `(shared P?)?` is its canonical optional owner and `shared? P?` is exact shorthand. Parsing, allocation recovery, exact optional targets, polymorphic class/base/interface/`Obj` static view identities, and the type-check availability gate are implemented. Typed construction, verified ownership, native realization, access, and stored-position integration remain with later roadmap tasks. |
