...
The implemented [function-value compiler contract](FUNCTION_VALUES.md) likewise
adds no runtime entry point, environment, allocation, owner operation, public
symbol, panic reason, or compatibility change. Its non-null code
...
Checked backing allocation and release continue through the existing raw-byte
boundary. Length evaluation, dynamic initialized-prefix state, index binding,
element expression execution, destination-typed initialization, per-element
cleanup, completion proof, and publication remain compiler and backend
...
allocation-header field, or ABI-version change. Every accepted application is
compiled as an ordinary exact class before HIR and MIR; its existing concrete
value, owner, array, optional, call, allocation, and lifecycle operations use
the same runtime boundary as equivalent hand-written types.

Specialized private symbols and code size are compiler/backend concerns. The
runtime receives neither a template identity nor an argument list. Existing
...
Compiler, backend, trace, linker, and native golden tests provide the feature
evidence. The direct runtime suite remains generic-agnostic because no runtime
service or public symbol was added.
