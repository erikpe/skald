# Low-Level Execution Declarations

Status: implemented private declaration and identity foundation. Production
emission still uses the [current backend](BACKEND.md). Lowered/selected
instruction graphs, phase publication, edits, placement and native consumption
of these declarations remain planned.
...
Service effect summaries and operation-level checks are not implemented yet.
...
Declaration tests are colocated under the plan owner; malformed arena tests are
colocated under the arena owner. They challenge foreign live contexts, wrong
...
cannot enter the declaration/arena core. Private APIs without native consumers
temporarily have item-scoped non-test lint allowances. Their removal obligations
