...
| Generic bodies retain all loop-body type uses; range endpoints that become known only after specialization request the exact key per closed body—even when distinct keys share one source span—and preserve nested discovery. | `specialized_generic_bodies_discover_deferred_and_nested_range_requests` and `deferred_generic_ranges_select_distinct_keys_for_the_same_source_span` in [range resolution tests](../../crates/skald-compiler/src/resolve/tests/range_language_item.rs), plus `nondependent_iteration_in_a_generic_body_retains_generic_type_uses` in [iteration resolution tests](../../crates/skald-compiler/src/resolve/tests/iteration.rs). |
...
| Normal completion, nesting, `continue`, `break`, and `return` preserve item, state, receiver, and body-local lifecycle. | The direct nested loop in `concise_ranges`, `nested_concise_ranges_reuse_one_completed_semantic_specialization` and `deeply_nested_concise_ranges_discover_new_endpoint_types_to_a_fixpoint` in [range resolution tests](../../crates/skald-compiler/src/resolve/tests/range_language_item.rs), `explicit_classes`, `verifier_rejects_fused_storage_operation_and_lifetime_mutations` in [range MIR tests](../../crates/skald-compiler/src/mir/tests/range_iteration.rs), and ordinary iteration lifecycle coverage. |
...
| Semantic request discovery exposes deterministic rounds, revisited callable bodies, and interner copies; each continuing round adds a specialization, while ordinary expressions perform no probe work. | `ordinary_expressions_skip_semantic_range_discovery`, `semantic_range_delta_follows_local_endpoint_bindings`, `nested_concise_ranges_reuse_one_completed_semantic_specialization`, and `deeply_nested_concise_ranges_discover_new_endpoint_types_to_a_fixpoint` in [range resolution tests](../../crates/skald-compiler/src/resolve/tests/range_language_item.rs), plus resolution detail-metric coverage in [reporting metric tests](../../crates/skald-compiler/src/driver/tests/reporting/metrics.rs). |
